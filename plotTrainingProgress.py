...
def setupArgParser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Plotted values are smoothed with a moving average. Specify over how many values (subepochs) it should extend. \nFormat: -m 20 (Default: 1)\n*NOTE* DSC from full-segmentation of validation images is not smoothed.")
    parser.add_argument(
        "-s",
        "--saveFigure",
        dest='save_figure',
        action='store_true',
        help="Use to make the script save the figure at the current folder. Takes no arguments.")
    return parser
...
def getSubepochsPerEpoch(pathToLog):
    lineWithPattern = getFirstLineInLogWithCertainPattern(
        pathToLog, SUBEPS_PER_EP_PATTERN)
    if lineWithPattern == None: return None
    return getIntFromStr(lineWithPattern[lineWithPattern.find(
        SUBEPS_PER_EP_PATTERN) + len(SUBEPS_PER_EP_PATTERN):])


def getEpochsBetweenFullInf(pathToLog):
    lineWithPattern = getFirstLineInLogWithCertainPattern(
        pathToLog, NUM_EPS_BETWEEN_FULLINF_PATTERN)
    if lineWithPattern == None: return None
    return getIntFromStr(lineWithPattern[lineWithPattern.find(
        NUM_EPS_BETWEEN_FULLINF_PATTERN) + len(
            NUM_EPS_BETWEEN_FULLINF_PATTERN):])


def getNumberOfClasses(pathToLog):
    lineWithPattern = getFirstLineInLogWithCertainPattern(
        pathToLog, NUM_OF_CLASSES_PATTERN)
    if lineWithPattern == None: return None
    return getIntFromStr(lineWithPattern[lineWithPattern.find(
        NUM_OF_CLASSES_PATTERN) + len(NUM_OF_CLASSES_PATTERN):])
...
def parseLogFileAndGetVariablesOfInterest(pathToLog):
    experimentName = None
    subepochsPerEpoch = None
    epochsBetweenEachFullInfer = None
...
def parseVariablesOfTrainingSessionsFromListOfLogs(inLogsList):
    listOfExperimentsNames = []
    listOfSubepochsPerEpFromEachLog = []
    listOfEpochsPerFullInferFromEachLog = []
    for log_i in xrange(len(inLogsList)):
        #Get variables from the logfile.
        (experimentName, subepochsPerEpoch, epochsBetweenEachFullInfer
         ) = parseLogFileAndGetVariablesOfInterest(inLogsList[log_i])
        ...
        if not epochsBetweenEachFullInfer:
    ...
    return (listOfExperimentsNames, listOfSubepochsPerEpFromEachLog,
            listOfEpochsPerFullInferFromEachLog)
...
def makeLegendList(listOfExperimentsNames, classesFromEachLogFile):
    ...
    for exper_i in xrange(len(listOfExperimentsNames)):
        for classFromExper in classesFromEachLogFile[exper_i]:
            legendList.append(listOfExperimentsNames[exper_i] + "-Class" + str(
                classFromExper))
    return legendList


def makeHelperVariablesPerExperiment(logFiles, classesFromEachLogFile,
                                     subepochsPerEpFromEachLog,
                                     epochsPerFullInferFromEachLog):
    subepochsPerEpOfExpers = []
    epochsPerFullInferOfExpers = []
    for logFile_i in xrange(len(logFiles)):
        for classForLogFile_i in xrange(len(classesFromEachLogFile[
                logFile_i])):
            subepochsPerEpOfExpers.append(subepochsPerEpFromEachLog[
                logFile_i])  # Essentially just doublicating the same entry again and again for all classes of same logfile/experiment.
            epochsPerFullInferOfExpers.append(epochsPerFullInferFromEachLog[
                logFile_i])
    return (subepochsPerEpOfExpers, epochsPerFullInferOfExpers)


def getStringOfTheListThatForSureStartsInThisLineButMayEndInAnother(
        restOfTheLineAfterTheEndOfTheWantedPattern, f):
    ...
    if indexWhereListEndsInThisLine > -1:
        theListInString = restOfTheLineAfterTheEndOfTheWantedPattern[
            indexWhereListStartsInThisLine:indexWhereListEndsInThisLine + 1]
        endOfListFound = True
    else:
        theListInString = restOfTheLineAfterTheEndOfTheWantedPattern[
            indexWhereListStartsInThisLine:]
...
def getRegExprForParsingMetric(validation0orTraining1, basic0detailed1,
                               class_i, intSpecifyingMetric01234):
...
def getListOfAccNumbersFromListOfStrNumbersAvoidingNotAppl(
    ...
    for stringNumber in listOfstringNumbers:
        ...
        parseFloatNumber = float(
            stringNumberStrippedOfWhiteSpace) if stringNumberStrippedOfWhiteSpace <> NA_PATTERN else previousValueOfTheVariableInTheTimeSerie
...
def movingAverageConv(a, window_size=1):
    ...
    result = np.convolve(
        a, window,
        'full')[:len(a)]  # Convolve full returns array of shape ( M + N - 1 ).
    slotsWithIncompleteConvolution = min(len(a), window_size - 1)
    result[slotsWithIncompleteConvolution:] = result[
        slotsWithIncompleteConvolution:] / float(window_size)
    if slotsWithIncompleteConvolution > 1:
        divisorArr = np.asarray(
            range(1, slotsWithIncompleteConvolution + 1, 1),
            dtype=float)
        result[:
               slotsWithIncompleteConvolution] = result[:
                                                        slotsWithIncompleteConvolution] / divisorArr
    return result
...
def applyMovingAverageToAllButDscFullSeg(
        detailedPlotBool, measuredMetricsFromAllExperiments, movingAverSubeps):
        ...
        for experimentToPlot in valOrTrainExperiments:  # Number of logs X Classes
            for metric_i in xrange(len(experimentToPlot)):
                if detailedPlotBool and metric_i == 4:  # We are plotting detailed metrics and this is the DSC-Full-Seg, which we do not smooth with movingAverage
                    continue
                experimentToPlot[metric_i] = movingAverageConv(
                    experimentToPlot[metric_i], movingAverSubeps)
...
def checkIfLineMatchesAnyRegExpr(string, regExprForEachClassAndMetric):
    for val0orTrain1 in xrange(len(regExprForEachClassAndMetric)):  #[0,1]
        for class_i in xrange(len(regExprForEachClassAndMetric[val0orTrain1])):
            for metric_i in xrange(len(regExprForEachClassAndMetric[
                    val0orTrain1][class_i])):
                regExp1 = regExprForEachClassAndMetric[val0orTrain1][class_i][
                    metric_i]
...
def parseBasicMetricsFromThisLog(logFile, movingAverSubeps):
    ...
    for val0orTrain1 in [0, 1]:
        (regExprForMeanAcc, sentenceForMeanAcc) = getRegExprForParsingMetric(
            val0orTrain1, 0, None, 0)
        regExprForEachClassAndMetric[val0orTrain1][0].append(regExprForMeanAcc)
        sentencesToLookForEachClassAndMetric[val0orTrain1][0].append(
            sentenceForMeanAcc)
    ...
    previousMeasurementForEachClassAndMetric = [
        [[]], [[]]
    ]  #This is useful in the case I get a not-valid number, to just use the previous one.
    for val0orTrain1 in [0, 1]:
        for metric_i in xrange(1):
            measurementsForEachClassAndMetric[val0orTrain1][0].append(
                [])  # Add a sublist in the class, per metric.
    ...
    newLine = f.readline()
    while newLine:
        matchObj, matchVal0Train1, matchClass_i, matchMetric_i = checkIfLineMatchesAnyRegExpr(
            newLine, regExprForEachClassAndMetric)

        if matchObj:  #matched the reg-expression for Acc
            sentenceToLookFor = sentencesToLookForEachClassAndMetric[
                matchVal0Train1][matchClass_i][matchMetric_i]
            restOfLineAfterPattern = newLine[newLine.find(sentenceToLookFor) +
                                             len(sentenceToLookFor):]
            theListInString = getStringOfTheListThatForSureStartsInThisLineButMayEndInAnother(
                restOfLineAfterPattern, f)
            listOfstringNumbersSplitted = getAListOfStringNumbersAfterSplittingThemFromAStringListWithStringNumbers(
                theListInString, "")

            previousMeasurementForClassAndMetric = previousMeasurementForEachClassAndMetric[
                matchVal0Train1][matchClass_i][matchMetric_i]
            listOfMeasurements = getListOfAccNumbersFromListOfStrNumbersAvoidingNotAppl(
                listOfstringNumbersSplitted,
                previousMeasurementForClassAndMetric)

            previousMeasurementForEachClassAndMetric[matchVal0Train1][
                matchClass_i][matchMetric_i] = listOfMeasurements[
                    -1]  # LHS use the list itself, not an intermediate immutable int-variable!
            measurementsForEachClassAndMetric[matchVal0Train1][matchClass_i][
                matchMetric_i] += listOfMeasurements

        newLine = f.readline()

    f.close()

    return (measurementsForEachClassAndMetric[0],
            measurementsForEachClassAndMetric[1])
...
def parseDetailedMetricsFromThisLog(logFile, classesFromThisLog,
                                    movingAverSubeps):
    ### Initially just form a data structure with the regular expressions for each val/train, class and metric. ###
    # [0] val, [1] train
    ...
    for val0orTrain1 in [0, 1]:
        for classInt in classesFromThisLog:
            ...
            for metric_i in xrange(len(regExprForClassAllMetrics)):
                (regExprForClassAllMetrics[metric_i],
                 sentencesForClassAllMetrics[metric_i]
                 ) = getRegExprForParsingMetric(val0orTrain1, 1, classInt,
                                                metric_i)
            regExprForEachClassAndMetric[val0orTrain1].append(
                regExprForClassAllMetrics)
            sentencesToLookForEachClassAndMetric[val0orTrain1].append(
    ...
    for val0orTrain1 in [0, 1]:
        for class_i in xrange(len(classesFromThisLog)):
            measurementsForEachClassAndMetric[val0orTrain1].append(
                [])  # add a sublist in the val/train for each class
            previousMeasurementForEachClassAndMetric[val0orTrain1].append([])
            for metric_i in xrange(0, 5):  # CAREFUL WITH THIS >> 5 <<
                measurementsForEachClassAndMetric[val0orTrain1][
                    class_i].append(
                        [])  # Add a sublist in the class, per metric.
                if metric_i == 4:  # If it's the DSC-full-segm, add an initial 0 measurement!
                    measurementsForEachClassAndMetric[val0orTrain1][class_i][
                        -1].append(0)
                previousMeasurementForEachClassAndMetric[val0orTrain1][
                    class_i].append(0)
    ...
    while newLine:
        ...
        if matchObj:  #matched one of the reg-expressions for Acc/Sens/Spec/Dsc-Samples.
            ...
            measurementsForEachClassAndMetric[matchVal0Train1][matchClass_i][
                ...
                regExprForDscFullSeg, newLine, re.M | re.
                I):  # Did not match the reg-expressions for Acc/Sens/Spec/Dsc-Samples. But matches DSC-Full-Inf!
            ...
            for class_i in xrange(len(classesFromThisLog)):
                previousMeasurement = previousMeasurementForEachClassAndMetric[
                    0][class_i][
                        4]  # get last value found for DSC of this class.
                dscForTheWantedClassInString = listOfstringNumbersSplitted[
                    classesFromThisLog[
                        class_i]]  # Reported list with DICE is different than others and has a float per class.
                # listOfMeasurements = [float], just a list with one float in this case of DSC-full-seg.
                listOfMeasurements = getListOfAccNumbersFromListOfStrNumbersAvoidingNotAppl(
                    [dscForTheWantedClassInString],
                    previousMeasurement)  # just returns the str number as float here.

                previousMeasurementForEachClassAndMetric[0][class_i][
                    4] = listOfMeasurements[
                        -1]  # DONT replace LHS with any intermediate Int immutable variable!
                measurementsForEachClassAndMetric[0][class_i][
                    4] += listOfMeasurements
...
def optimizedParseMetricsFromLogs(logFiles, detailedPlotBool,
                                  classesFromEachLogFile, movingAverSubeps):
    ...
    measuredMetricsFromAllExperiments = [
        [], []
    ]  #[0] validation, [1] training measurements.
    for logFile_i in xrange(0, len(logFiles)):
        if not detailedPlotBool:
            (measuredMetricsFromThisLogValidation,
             measuredMetricsFromThisLogTraining
             ) = parseBasicMetricsFromThisLog(logFiles[logFile_i],
                                              movingAverSubeps)
        else:
            (measuredMetricsFromThisLogValidation,
             measuredMetricsFromThisLogTraining
             ) = parseDetailedMetricsFromThisLog(
                 logFiles[logFile_i], classesFromEachLogFile[logFile_i],
                 movingAverSubeps)

        measuredMetricsFromAllExperiments[
            0] += measuredMetricsFromThisLogValidation
        measuredMetricsFromAllExperiments[
            1] += measuredMetricsFromThisLogTraining
    measuredMetricsFromAllExperiments = applyMovingAverageToAllButDscFullSeg(
        detailedPlotBool, measuredMetricsFromAllExperiments, movingAverSubeps)
    return measuredMetricsFromAllExperiments
...
def plotProgressBasic(measuredMetricsFromAllExperiments, legendList,
                      movingAverSubeps, subepochsPerEpOfExpers,
                      saveFigureBool):
    colors = ["r", "g", "b", "c", "m", "k"]
    linestyles = ['-', '--', ':', '_', '-.']
    ...
    fontSizeSubplotTitles = 14
    fontSizeXTickLabel = 12
    fontSizeYTickLabel = 12
    fontSizeXAxisLabel = 12
    fontSizeYAxisLabel = 14
    linewidthInPlots = 1.5
    legendFontSize = 12
    legendNumberOfColumns = 8
    #plt.close('all')
    ...
    fig, axes = plt.subplots(
        2, numberOfMetricsPlotted,
        sharex=False, sharey=False)
    inchesForMainPlotPart = 7
    inchesForLegend = 0.6
    percForMain = inchesForMainPlotPart * 1.0 / (
        inchesForMainPlotPart + inchesForLegend)
    percForLegend = 1. - percForMain
    fig.set_size_inches(15, inchesForMainPlotPart + inchesForLegend)
    #changes width/height of the figure. VERY IMPORTANT
    fig.set_dpi(100)
    #changes width/height of the figure.

    fig.subplots_adjust(left=0.05,
                        bottom=0.1 * percForMain + percForLegend,
                        right=0.98,
                        top=0.92 * percForMain + percForLegend,
                        wspace=0.25,
                        hspace=0.4 * percForMain)
    fig.canvas.set_window_title(os.path.basename(__file__))
    fig.suptitle(
        os.path.basename(__file__) + ": Moving Average over [" +
        str(movingAverSubeps) +
        "] value. For each plotted experiment, Subepochs per Epoch: " +
        str(subepochsPerEpOfExpers),
    ...
    for valOrTrain_i in xrange(0, len(measuredMetricsFromAllExperiments)):
        for valOrTrainExperiment_i in xrange(
                0, len(measuredMetricsFromAllExperiments[valOrTrain_i])):
            valOrTrainExperiment = measuredMetricsFromAllExperiments[
                valOrTrain_i][valOrTrainExperiment_i]
            for metric_i in xrange(0, len(valOrTrainExperiment)):
                numberOfSubsPerEpoch = subepochsPerEpOfExpers[
                    valOrTrainExperiment_i]
                ...
                xIter = np.linspace(
                    0,
                    numberOfEpochsRan,
                    numberOfSubepochsRan,
                    endpoint=True)  #endpoint=True includes it as the final point.

                axis = axes[
                    valOrTrain_i] if numberOfMetricsPlotted == 1 else axes[
                        valOrTrain_i,
                        metric_i]  # No 2nd index when subplot(X, 1, ...)
                axis.plot(xIter,
                          valOrTrainExperiment[metric_i],
                          color=colors[valOrTrainExperiment_i % len(colors)],
                          linestyle=linestyles[valOrTrainExperiment_i /
                                               len(colors)],
                          label=legendList[valOrTrainExperiment_i],
                          linewidth=linewidthInPlots)
                axis.set_title(subplotTitles[valOrTrain_i][metric_i],
                               fontsize=fontSizeSubplotTitles, y=1.022)
    ...
    for train0AndValidation1 in [0, 1]:
        axis = axes[
            train0AndValidation1] if numberOfMetricsPlotted == 1 else axes[
                train0AndValidation1][axis_i]
    ...
    if saveFigureBool:
        plt.savefig('./trainingProgress.pdf',
                    dpi=fig.dpi)  #, bbox_inches='tight')

    plt.show()


def plotProgressDetailed(measuredMetricsFromAllExperiments, legendList,
                         movingAverSubeps, subepochsPerEpOfExpers,
                         epochsPerFullInferOfExpers, saveFigureBool):
    colors = ["r", "g", "b", "c", "m", "k"]
    linestyles = ['-', '--', ':', '_', '-.']

    subplotTitles = [["Mean Accuracy", "Sensitivity", "Specificity",
                      "DSC (samples)", "DSC (full-segm)"],  # Validation
                     ["Mean Accuracy", "Sensitivity", "Specificity",
                      "DSC (samples)", "DSC (full-segm)"]  # Training
                     ]
    ...
    fig.suptitle(
        ...
        str(movingAverSubeps) +
        "] value. For each plotted experiment, Subepochs per Epoch: " +
        str(subepochsPerEpOfExpers) + ", Epochs between Full-Segmentations: " +
        str(epochsPerFullInferOfExpers),
        fontsize=8)  #, fontweight='bold')
    ...
    for valOrTrain_i in xrange(0, len(measuredMetricsFromAllExperiments)):
        for valOrTrainExperiment_i in xrange(
            ...
            for meanPosNegDice1_i in xrange(0, len(valOrTrainExperiment)):
                numberOfSubsPerEpoch = subepochsPerEpOfExpers[
                    valOrTrainExperiment_i]
                numberOfEpsBetweenFullInf = epochsPerFullInferOfExpers[
                    valOrTrainExperiment_i]

                if meanPosNegDice1_i <> 4:  #Not for DSC full inference.
                    numberOfSubepochsRan = len(valOrTrainExperiment[
                        meanPosNegDice1_i])
                    numberOfEpochsRan = numberOfSubepochsRan * 1.0 / numberOfSubsPerEpoch
                    maxNumOfEpsDurationOfExps = maxNumOfEpsDurationOfExps if maxNumOfEpsDurationOfExps >= numberOfEpochsRan else numberOfEpochsRan
                    xIter = np.linspace(
                    ...
                    numberOfFullInfRanPlusOneAt0 = len(valOrTrainExperiment[
                        meanPosNegDice1_i])
                    numberOfEpochsRan = (numberOfFullInfRanPlusOneAt0 -
                                         1) * numberOfEpsBetweenFullInf
                    maxNumOfEpsDurationOfExps = maxNumOfEpsDurationOfExps if maxNumOfEpsDurationOfExps >= numberOfEpochsRan else numberOfEpochsRan
                    xIter = np.linspace(0,
                                        numberOfEpochsRan,
                                        numberOfFullInfRanPlusOneAt0,
                                        endpoint=True)

                axes[valOrTrain_i, meanPosNegDice1_i].plot(
                    xIter,
                    valOrTrainExperiment[meanPosNegDice1_i],
                    color=colors[valOrTrainExperiment_i % len(colors)],
                    linestyle=linestyles[valOrTrainExperiment_i / len(colors)],
                    label=legendList[valOrTrainExperiment_i],
                    linewidth=linewidthInPlots)
                axes[valOrTrain_i, meanPosNegDice1_i].set_title(
                    subplotTitles[valOrTrain_i][meanPosNegDice1_i],
                    fontsize=fontSizeSubplotTitles, y=1.022)
                axes[valOrTrain_i, meanPosNegDice1_i].yaxis.grid(True,
                                                                 zorder=0)
                axes[valOrTrain_i, meanPosNegDice1_i].set_xlim(
                    [0, maxNumOfEpsDurationOfExps])
                axes[valOrTrain_i, meanPosNegDice1_i].set_xlabel(
                    'Epoch', fontsize=fontSizeXAxisLabel)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        print(
            "For help on the usage of this script, please use the option [-h]."
        )
        exit(1)
    ...
    (listOfExperimentsNames, subepochsPerEpFromEachLog,
     epochsPerFullInferFromEachLog
     ) = parseVariablesOfTrainingSessionsFromListOfLogs(logFiles)
    ...
    if not detailedPlotBool:  # basic plot
        ...
        measuredMetricsFromAllExperiments = optimizedParseMetricsFromLogs(
            logFiles, detailedPlotBool, None, movingAverSubeps)
        plotProgressBasic(measuredMetricsFromAllExperiments,
                          listOfExperimentsNames, movingAverSubeps,
                          subepochsPerEpFromEachLog, saveFigBool)
        ...
        if not args.classes_to_plot:  #Default class when none given as argument
            ...
            classesFromEachLogFile = [args.classes_to_plot
                                      ]  # [ [class0, class1, ...] ]
        elif len(
                args.
                classes_to_plot) == 1:  # multiple logs provided, and 1 class argument
            classesFromEachLogFile = [[args.classes_to_plot[0]]
                                      for i in xrange(len(logFiles))
                                      ]  # [ [class0], [class0], ... ]
        elif len(args.classes_to_plot) == len(logFiles):
            classesFromEachLogFile = [[args.classes_to_plot[i]]
                                      for i in xrange(len(logFiles))
                                      ]  # [ [class0], [class1], [class2], ...]
        else:  # logFiles provided, multiple classes provided, but not the same number as the log files.
            print(
                "ERROR:\tThe number of log files given is not the same with the number of arguments that specify which class's accuracy to plot from each log file."
            )
            print(
                "\tPlease provide the same number of Class arguments, or just 1, if the same class is to be plotted from all log files. Exiting."
            )
            exit(1)
        ...
        """
        measuredMetricsFromAllExperiments = optimizedParseMetricsFromLogs(
            logFiles, detailedPlotBool, classesFromEachLogFile,
            movingAverSubeps)
        legendList = makeLegendList(listOfExperimentsNames,
                                    classesFromEachLogFile)
        (subepochsPerEpOfExpers,
         epochsPerFullInferOfExpers) = makeHelperVariablesPerExperiment(
             logFiles, classesFromEachLogFile, subepochsPerEpFromEachLog,
             epochsPerFullInferFromEachLog)

        plotProgressDetailed(measuredMetricsFromAllExperiments, legendList,
                             movingAverSubeps, subepochsPerEpOfExpers,
                             epochsPerFullInferOfExpers, saveFigBool)
