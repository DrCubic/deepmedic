...
from deepmedic.cnnHelpers import \
...
from deepmedic.pathways import FcPathway, NormalPathway, SubsampledPathway
...
def padImageWithMirroring(inputImage, voxelsPerDimToPad):
    ...
    paddedImage = T.concatenate(
        [inputImage[:, :, int(voxelsPerDimToPad[0] / 2) - 1::-1, :, :],
         inputImage],
        axis=2) if padLeft > 0 else inputImage
    paddedImage = T.concatenate(
        [paddedImage, paddedImage[:, :, -1:-1 - int((voxelsPerDimToPad[0] + 1)
                                                    / 2):-1, :, :]],
    ...
    paddedImage = T.concatenate(
        [paddedImage[:, :, :, padLeft - 1::-1, :], paddedImage],
        axis=3) if padLeft > 0 else paddedImage
    paddedImage = T.concatenate(
        [paddedImage, paddedImage[:, :, :, -1:-1 - padRight:-1, :]],
...
class Cnn3d(object):
    def __init__(self):
        ...
        self.learning_rate = theano.shared(np.cast["float32"](
            0.01))  # initial value, changed in make_cnn_model().compileTrainingFunction()
        ...
        self.i_adam = theano.shared(
            np.cast["float32"](0.))  # Current iteration of adam
        self.m_listForAllParamsAdam = [
        ]  # list of mean of grads for all parameters, for ADAM optimizer.
        self.v_listForAllParamsAdam = [
        ]  # list of variances of grads for all parameters, for ADAM optimizer.
        # RMSProp
    ...
    def change_learning_rate_of_a_cnn(self,
                                      newValueForLearningRate,
                                      myLogger=None):
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(
            self.
            numberOfEpochsTrained) + ") Changing the Cnn's Learning Rate to: " + str(
                newValueForLearningRate)
        if myLogger <> None:
    ...
    def change_momentum_of_a_cnn(self, newValueForMomentum, myLogger=None):
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(
            self.
            numberOfEpochsTrained) + ") Changing the Cnn's Momentum to: " + str(
                newValueForMomentum)
        if myLogger <> None:
            myLogger.print3(stringToPrint)
        else:
    ...
    def changeB1AndB2ParametersOfAdam(self, b1ParamForAdam, b2ParamForAdam,
                                      myLogger):
        myLogger.print3(
            "UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) +
    ...
    def changeRhoParameterOfRmsProp(self, rhoParamForRmsProp, myLogger):
        myLogger.print3(
            "UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) +
            ") Changing the Cnn's Rho parameter for RMSProp optimization to: Rho = "
            + str(rhoParamForRmsProp))
        self.rho_rmsProp = rhoParamForRmsProp

    def checkMeanValidationAccOfLastEpochAndUpdateCnnsTopAccAchievedIfNeeded(
            self, myLogger, meanValidationAccuracyOfLastEpoch,
        ...
        highestAchievedValidationAccuracyOfCnn = self.topMeanValidationAccuracyAchievedInEpoch[
            0]
        if meanValidationAccuracyOfLastEpoch > highestAchievedValidationAccuracyOfCnn + minIncreaseInValidationAccuracyConsideredForLrSchedule:
            self.topMeanValidationAccuracyAchievedInEpoch[
                0] = meanValidationAccuracyOfLastEpoch
            self.topMeanValidationAccuracyAchievedInEpoch[
                1] = self.numberOfEpochsTrained
            myLogger.print3(
                "UPDATE: In this last epoch (cnnTrained) #" + str(
                    self.topMeanValidationAccuracyAchievedInEpoch[1]) +
    ...
    def _initializeSharedVarsForInputsTrain(self):
        ...
        self.sharedInpXTrain = theano.shared(
            np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"),
            borrow=self.borrowFlag)
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListTrain.append(theano.shared(
                np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"),
                borrow=self.borrowFlag))
        ...
        self.sharedLabelsYTrain = theano.shared(
            np.zeros(
    ...
    def _initializeSharedVarsForInputsVal(self):
        ...
        self.sharedInpXVal = theano.shared(
            np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"),
            borrow=self.borrowFlag)
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListVal.append(theano.shared(
                np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"),
                borrow=self.borrowFlag))
        self.sharedLabelsYVal = theano.shared(
            np.zeros(
                [1, 1, 1, 1], dtype="float32"),
            borrow=self.borrowFlag)
    ...
    def _initializeSharedVarsForInputsTest(self):
        ...
        self.sharedInpXTest = theano.shared(
            np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"),
            borrow=self.borrowFlag)
        for subsPath_i in xrange(self.numSubsPaths):
            self.sharedInpXPerSubsListTest.append(theano.shared(
                np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"),
                borrow=self.borrowFlag))
    ...
    def freeGpuTrainingData(self):
        if self._initializedSharedVarsTrain:  # False if this is called (eg save model) before train/val/test function is compiled.
            self.sharedInpXTrain.set_value(np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"))  # = []
            for subsPath_i in xrange(self.numSubsPaths):
                self.sharedInpXPerSubsListTrain[subsPath_i].set_value(np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"))
            self.sharedLabelsYTrain.set_value(np.zeros(
                [1, 1, 1, 1], dtype="float32"))  # = []

    def freeGpuValidationData(self):
        if self._initializedSharedVarsVal:
            self.sharedInpXVal.set_value(np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"))  # = []
            for subsPath_i in xrange(self.numSubsPaths):
                self.sharedInpXPerSubsListVal[subsPath_i].set_value(np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"))
            self.sharedLabelsYVal.set_value(np.zeros(
                [1, 1, 1, 1], dtype="float32"))  # = []

    def freeGpuTestingData(self):
        if self._initializedSharedVarsTest:
            self.sharedInpXTest.set_value(np.zeros(
                [1, 1, 1, 1, 1], dtype="float32"))  # = []
            for subsPath_i in xrange(self.numSubsPaths):
                self.sharedInpXPerSubsListTest[subsPath_i].set_value(np.zeros(
                    [1, 1, 1, 1, 1], dtype="float32"))
    ...
    def updateTheMatricesOfTheLayersWithTheLastMusAndVarsForTheMovingAverageOfTheBatchNormInference(
            self):
    ...
    def getUpdatesAccordingToSgd(self, cost, paramsToOptDuringTraining):
        ...
        for param, grad, v in zip(paramsToOptDuringTraining, grads,
                                  self.velocities_forMom):
            ...
            updates.append(
                (v, newVelocity))  # I can do (1-mom)*learnRate*grad.
    ...
    def _initializeSharedVariablesOfRmsProp(self, paramsToOptDuringTraining):
        ...
        for param in paramsToOptDuringTraining:
            accu = theano.shared(
                param.get_value() * 0.,
                broadcastable=param.
                broadcastable)  # accumulates the mean of the grad's square.
            self.accuGradSquare_listForAllParamsRmsProp.append(accu)
            v = theano.shared(param.get_value() * 0.,
                              broadcastable=param.broadcastable)  # velocity
            self.velocities_forMom.append(v)
    ...
    def getUpdatesAccordingToRmsProp(self, cost, params):
        ...
        for param, grad, accu, v in zip(
                params, grads, self.accuGradSquare_listForAllParamsRmsProp,
                self.velocities_forMom):
            accu_new = self.rho_rmsProp * accu + (
                1 - self.rho_rmsProp) * T.sqr(grad)
            stepToGradientDirection = multiplierForCurrentGradUpdateForNonNormalizedOrNormalizedMomentum * (
                self.learning_rate * grad /
                T.sqrt(accu_new + self.epsilonForRmsProp))
            newVelocity = self.momentum * v - stepToGradientDirection
    ...
    def _initializeSharedVariablesOfAdam(self, paramsToOptDuringTraining):
        self.i_adam = theano.shared(
            np.cast["float32"](0.))  # Current iteration
        self.m_listForAllParamsAdam = [
        ]  # list of mean of grads for all parameters, for ADAM optimizer.
        self.v_listForAllParamsAdam = [
        ]  # list of variances of grads for all parameters, for ADAM optimizer.
    ...
    def getUpdatesAccordingToAdam(self, cost, params):
        ...
        for param, grad, m, v in zip(params, grads,
                                     self.m_listForAllParamsAdam,
                                     self.v_listForAllParamsAdam):
            m_t = (self.b1_adam * m) + ((1. - self.b1_adam) * grad)
            v_t = (self.b2_adam * v) + ((1. - self.b2_adam) * T.sqr(grad)
                                        )  # Double check this with the paper.
    ...
    def _getUpdatesOfTrainableParameters(self, myLogger, cost):
        ...
        if self.sgd0orAdam1orRmsProp2 == 0:
            myLogger.print3(
                "Optimizer used: [SGD]. Momentum used: Classic0 or Nesterov1 : "
                + str(self.classicMomentum0OrNesterov1))
            updates = self.getUpdatesAccordingToSgd(cost,
                                                    paramsToOptDuringTraining)
        elif self.sgd0orAdam1orRmsProp2 == 1:
            myLogger.print3(
                "Optimizer used: [ADAM]. No momentum implemented for Adam.")
            updates = self.getUpdatesAccordingToAdam(cost,
                                                     paramsToOptDuringTraining)
        elif self.sgd0orAdam1orRmsProp2 == 2:
            myLogger.print3(
                "Optimizer used: [RMSProp]. Momentum used: Classic0 or Nesterov1 : "
                + str(self.classicMomentum0OrNesterov1))
            updates = self.getUpdatesAccordingToRmsProp(
                cost, paramsToOptDuringTraining)
        return updates
    ...
    def _getTrainableParameters(self, myLogger):
        ...
        for pathway in self.pathways:
            for layer_i in xrange(0, len(pathway.getLayers())):
                if layer_i not in self.indicesOfLayersPerPathwayTypeToFreeze[
                        pathway.pType()]:
                    ...
                    myLogger.print3(
                        "WARN: [Pathway_" + str(pathway.getStringType(
    ...
    def initializeTrainingState(
            self, myLogger, indicesOfLayersPerPathwayTypeToFreeze,
            costFunctionLetter, learning_rate, sgd0orAdam1orRmsProp2,
            classicMomentum0OrNesterov1, momentum,
            momentumTypeNONNormalized0orNormalized1, b1ParamForAdam,
            b2ParamForAdam, epsilonForAdam, rhoParamForRmsProp,
            epsilonForRmsProp, L1_reg_constant, L2_reg_constant):
        ...
        if sgd0orAdam1orRmsProp2 == 1:
            self.changeB1AndB2ParametersOfAdam(b1ParamForAdam, b2ParamForAdam,
                                               myLogger)
        ...
        for pathway in self.pathways:
            for layer in pathway.getLayers():
                updatesForBnRollingAverage.extend(
                    layer.getUpdatesForBnRollingAverage(
    ...
    def compileTrainFunction(self, myLogger):
        ...
        y = T.itensor4(
            'y')  # Input of the theano-compiled-function. Dimensions of y labels: [batchSize, r, c, z]
        ...
        if self.costFunctionLetter == "L":
            costFromLastLayer = self.finalTargetLayer.negativeLogLikelihood(
                y, weightPerClass)
        else:
            myLogger.print3(
        ...
        givensSet = {x: self.sharedInpXTrain[index * self.batchSize:(index + 1)
                                             * self.batchSize]}
        for subPath_i in xrange(
                self.numSubsPaths):  # if there are subsampled paths...
            xSub = listXPerSubs[subPath_i]
            sharedInpXSubTrain = self.sharedInpXPerSubsListTrain[subPath_i]
            givensSet.update({xSub: sharedInpXSubTrain[index * self.batchSize:(
                index + 1) * self.batchSize]})
        givensSet.update(
            {y: intCastSharedLabelsYTrain[index * self.batchSize:(index + 1) *
                                          self.batchSize],
             weightPerClass: inputVectorWeightsOfClassesInCostFunction})

        myLogger.print3(
            "...Compiling the function for training... (This may take a few minutes...)")
    ...
    def compileValidationFunction(self, myLogger):
        ...
        givensSet = {x: self.sharedInpXVal[index * self.batchSizeValidation:(
            index + 1) * self.batchSizeValidation]}
        for subPath_i in xrange(
                self.numSubsPaths):  # if there are subsampled paths...
            xSub = listXPerSubs[subPath_i]
            sharedInpXSubVal = self.sharedInpXPerSubsListVal[subPath_i]
            givensSet.update({xSub: sharedInpXSubVal[
                index * self.batchSizeValidation:(
                    index + 1) * self.batchSizeValidation]})
        givensSet.update({y: intCastSharedLabelsYVal[
            index * self.batchSizeValidation:(index + 1
                                              ) * self.batchSizeValidation]})

        myLogger.print3(
            "...Compiling the function for validation... (This may take a few minutes...)")
    ...
    def compileTestAndVisualisationFunction(self, myLogger):
        ...
        for pathway in self.pathways:
            for layer in pathway.getLayers(
            ):  # each layer that this pathway/fc has.
                listToReturnWithAllTheFmActivationsAndPredictionsAppended.append(
                    layer.fmsActivations([0, 9999]))

        listToReturnWithAllTheFmActivationsAndPredictionsAppended.append(
            self.finalTargetLayer.predictionProbabilities())

        givensSet = {x: self.sharedInpXTest[index * self.batchSizeTesting:(
            index + 1) * self.batchSizeTesting]}
        for subPath_i in xrange(
                self.numSubsPaths):  # if there are subsampled paths...
            xSub = listXPerSubs[subPath_i]
            sharedInpXSubTest = self.sharedInpXPerSubsListTest[subPath_i]
            givensSet.update({xSub: sharedInpXSubTest[
                index * self.batchSizeTesting:(index + 1
                                               ) * self.batchSizeTesting]})
        ...
        self.cnnTestAndVisualiseAllFmsFunction = theano.function(
            ...
            givens=givensSet)
        myLogger.print3(
            "The function for testing and visualisation of FMs was compiled.")
    ...
    def _getInputTensorsXToCnn(self):
        if not self.inputTensorsXToCnnInitialized:
            ...
            for subsPath_i in xrange(self.numSubsPaths):
                self.listInputTensorPerSubsTrain.append(tensor5(
                ))  # Actually, for these 3, a single tensor5() could be used.
                self.listInputTensorPerSubsVal.append(tensor5(
                ))  # myTensor.reshape(inputImageSubsampledShapeValidation)
                self.listInputTensorPerSubsTest.append(tensor5())
            self.inputTensorsXToCnnInitialized = True

        return (self.inputTensorNormTrain, self.inputTensorNormVal,
                self.inputTensorNormTest, self.listInputTensorPerSubsTrain,
                self.listInputTensorPerSubsVal,
                self.listInputTensorPerSubsTest)
    ...
    def make_cnn_model( self,
        ...
        self.numSubsPaths = len(
            subsampleFactorsPerSubPath)  # do I want this as attribute? Or function is ok?
        ...
        self.recFieldCnn = calculateReceptiveFieldDimensionsFromKernelsDimListPerLayerForFullyConvCnnWithStrides1(
            kernelDimensions)
        ...
        inputToPathwayShapeTrain = [self.batchSize, numberOfImageChannelsPath1
                                    ] + imagePartDimensionsTraining
        ...
        thisPathwayUseBnPerLayer = [
            rollingAverageForBatchNormalizationOverThatManyBatches > 0
        ...
        thisPathwayActivFuncPerLayer = [activationFunctionToUseRelu0orPrelu1
                                        ] * thisPathwayNumOfLayers
        thisPathwayActivFuncPerLayer[
            0] = -1 if thisPathwayType <> pt.FC else activationFunctionToUseRelu0orPrelu1  # To not apply activation on raw input. -1 is linear activation.

        thisPathway.makeLayersOfThisPathwayAndReturnDimensionsOfOutputFM(
            myLogger, inputToPathwayTrain, inputToPathwayVal,
        ...
        for subPath_i in xrange(self.numSubsPaths):
            thisPathway = SubsampledPathway(subsampleFactorsPerSubPath[
                subPath_i])
            ...
            inputToPathwayShapeTrain = [
                self.batchSize, numberOfImageChannelsPath2
            ...
            inputToPathwayShapeVal = [
                self.batchSizeValidation, numberOfImageChannelsPath2
            ] + thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
                thisPathWayKernelDimensions, dimsOfOutputFrom1stPathwayVal)
            inputToPathwayShapeTest = [
                self.batchSizeTesting, numberOfImageChannelsPath2
            ] + thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
        ...
        myLogger.print3(
            "DEBUG: Shape of the kernel of the first FC layer is : " + str(
        ...
        self.finalTargetLayer.makeLayer(rng, self.getFcPathway().getLayer(-1),
                                        softmaxTemperature)
