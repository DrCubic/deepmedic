...
from deepmedic.frontEndModules.frontEndHelpers.createModelParametersClass import \
    CreateModelSessionParameters
...
from deepmedic.frontEndModules.frontEndHelpers.preparationForSessionHelpers import \
    makeFoldersNeededForCreateModelSession
from deepmedic.genericHelpers import (datetimeNowAsStr,
                                      load_object_from_gzip_file)
...
def deepMedicNewModelMain(modelConfigFilepath,
    ...
    modelName = configGet(modelConfig.MODEL_NAME) if configGet(
        modelConfig.
        MODEL_NAME) else CreateModelSessionParameters.getDefaultModelName()
    ...
    if absPathToPreTrainedModelGivenInCmdLine <> None:  # Transfer parameters from a previously trained model to the new one.
        ...
        cnnPretrainedInstance = load_object_from_gzip_file(
            absPathToPreTrainedModelGivenInCmdLine)
        sessionLogger.print3(
            "The pre-trained model was loaded successfully from: " + str(
                absPathToPreTrainedModelGivenInCmdLine))
    ...
    createModelSessionParameters.sessionLogger.print3(
        "=========== Creation of the model: \"" + str(
            createModelSessionParameters.cnnModelName) +
        "\" finished =================")
