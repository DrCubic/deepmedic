...
class TrainSessionParameters(object):
    ...
    def __init__(
        ...
        if self.useDefaultTrainingSamplingFromGtAndRoi:
            self.samplingTypeInstanceTrain = samplingType.SamplingType(
                self.sessionLogger, DEFAULT_SAMPLING_TYPE_TR,
            ...
            self.samplingTypeInstanceTrain.setPercentOfSamplesPerCategoryToSample(
                [1.0 / numberOfCategoriesOfSamplesTr] *
                numberOfCategoriesOfSamplesTr)
            ...
            self.samplingTypeInstanceTrain = samplingType.SamplingType(
                self.sessionLogger, samplingTypeToUseTr,
                self.cnn3dInstance.numberOfOutputClasses)
            if samplingTypeToUseTr in [
                    0, 3
            ] and proportionOfSamplesPerCategoryTrain:
                self.samplingTypeInstanceTrain.setPercentOfSamplesPerCategoryToSample(
                    proportionOfSamplesPerCategoryTrain)
            else:
                numberOfCategoriesOfSamplesTr = self.samplingTypeInstanceTrain.getNumberOfCategoriesToSample(
        ...
        #~~~~~~~~~~~~~~ Augmentation~~~~~~~~~~~~~~
        self.reflectImagesPerAxis = reflectImagesPerAxis if reflectImagesPerAxis else [
            False, False, False
        ]
        self.performIntAugm = performIntAugm if performIntAugm <> None else False
        if self.performIntAugm:
            self.sampleIntAugmShiftWithMuAndStd = sampleIntAugmShiftWithMuAndStd if sampleIntAugmShiftWithMuAndStd else [
                0.0, 0.1
            ]
            self.sampleIntAugmMultiWithMuAndStd = sampleIntAugmMultiWithMuAndStd if sampleIntAugmMultiWithMuAndStd else [
                1.0, 0.0
            ]
            self.intAugmOnPairs2Images1None0_imAlreadyNormal_ShiftMuStd_MultiMuStd = [
                2, 1, self.sampleIntAugmShiftWithMuAndStd,
                self.sampleIntAugmMultiWithMuAndStd
            ]
        else:
            self.intAugmOnPairs2Images1None0_imAlreadyNormal_ShiftMuStd_MultiMuStd = [
                0, "plcholder", [], []
        ...
        if self.useDefaultUniformValidationSampling:
            self.samplingTypeInstanceVal = samplingType.SamplingType(
                self.sessionLogger, DEFAULT_SAMPLING_TYPE_VAL,
                self.cnn3dInstance.numberOfOutputClasses)
            ...
            self.samplingTypeInstanceVal.setPercentOfSamplesPerCategoryToSample(
                [1.0 / numberOfCategoriesOfSamplesVal] *
            ...
            self.samplingTypeInstanceVal = samplingType.SamplingType(
                self.sessionLogger, samplingTypeToUseVal,
                self.cnn3dInstance.numberOfOutputClasses)
            if samplingTypeToUseVal in [
                    0, 3
            ] and proportionOfSamplesPerCategoryVal:
                self.samplingTypeInstanceVal.setPercentOfSamplesPerCategoryToSample(
                    proportionOfSamplesPerCategoryVal)
            else:
                numberOfCategoriesOfSamplesVal = self.samplingTypeInstanceVal.getNumberOfCategoriesToSample(
        ...
        self.saveProbMapsBoolPerClassVal = saveProbMapsBoolPerClassVal if (
            saveProbMapsBoolPerClassVal <> None and saveProbMapsBoolPerClassVal
            <> []) else [True] * cnn3dInstance.numberOfOutputClasses
        ...
        self.indicesOfFmsToVisualisePerPathwayAndLayerVal = [
            ...
            for item in indicesOfFmsToVisualisePerPathwayAndLayerVal
        ...
        if not self.namesToSavePredictionsAndFeaturesVal and self.performFullInferenceOnValidationImagesEveryFewEpochs and (
                self.saveSegmentationVal or
                True in self.saveProbMapsBoolPerClassVal or
                self.saveIndividualFmImagesVal or
        ...
        self.smoothChannelsWithGaussFilteringStdsForNormalAndSubsampledImage = [
            None, None
        ]  # One element for Normal and one for Subsampled Pathway. Either None for no smoothing, or a List [std-r, std-c, std-z] of the gaussian kernel to smooth with.
        ...
        if self.optimizerSgd0Adam1Rms2 == 0:
            ...
            self.eRms = eRms if eRms <> None else 10**(
                -4
        ...
        indicesOfLayersToFreezeNorm = [
            l - 1 for l in layersToFreezePerPathwayType[0]
        ] if layersToFreezePerPathwayType[0] <> None else []
        indicesOfLayersToFreezeSubs = [
            l - 1 for l in layersToFreezePerPathwayType[1]
        ] if layersToFreezePerPathwayType[
            1] <> None else indicesOfLayersToFreezeNorm
        indicesOfLayersToFreezeFc = [
            l - 1 for l in layersToFreezePerPathwayType[2]
        ] if layersToFreezePerPathwayType[2] <> None else []
    ...
    def _makeFilepathsForPredictionsAndFeaturesVal(
            self, absPathToFolderForPredictionsFromSession,
        ...
        if self.namesToSavePredictionsAndFeaturesVal <> None:
            for case_i in xrange(self.numberOfCasesVal):
                filepathForCasePrediction = absPathToFolderForPredictionsFromSession + "/" + self.namesToSavePredictionsAndFeaturesVal[
                    case_i]
                self.filepathsToSavePredictionsForEachPatientVal.append(
                    filepathForCasePrediction)
                filepathForCaseFeatures = absPathToFolderForFeaturesFromSession + "/" + self.namesToSavePredictionsAndFeaturesVal[
                    case_i]
                self.filepathsToSaveFeaturesForEachPatientVal.append(
                    filepathForCaseFeatures)
    ...
    def printParametersOfThisSession(self):
        ...
        logPrint("Path and filename to save trained models = " + str(
            self.pathAndFilenameToSaveTrainedModels))

        logPrint("~~~~~~~~~~~~~~~~~~Generic Information~~~~~~~~~~~~~~~~")
        logPrint("Number of Classes (incl. background) (from cnnModel) = " +
                 str(self.numberOfClasses))
        logPrint("Number of Channels (Normal Pathway) (from cnnModel) = " +
                 str(self.numberOfChannels))
        logPrint("Number of Cases for Training = " + str(
            self.numberOfCasesTrain))
        logPrint("Number of Cases for Validation = " + str(
            self.numberOfCasesVal))

        logPrint("~~~~~~~~~~~~~~~~~~Training parameters~~~~~~~~~~~~~~~~")
        logPrint("Filepaths to Channels of the Training Cases = " + str(
            self.channelsFilepathsTrain))
        logPrint("Filepaths to Ground-Truth labels of the Training Cases = " +
                 str(self.gtLabelsFilepathsTrain))

        logPrint("~~Sampling~~")
        logPrint("Region-Of-Interest Masks provided = " + str(
            self.providedRoiMasksTrain))
        logPrint("Filepaths to ROI Masks of the Training Cases = " + str(
            self.roiMasksFilepathsTrain))

        logPrint("~~Advanced Sampling~~")
        logPrint(
            "Using default sampling = " + str(
                self.useDefaultTrainingSamplingFromGtAndRoi) +
            ". NOTE: Adv.Sampl.Params are auto-set to perform default sampling if True.")
        logPrint("Type of Sampling = " + str(
            self.samplingTypeInstanceTrain.getStringOfSamplingType()) + " (" +
                 str(self.samplingTypeInstanceTrain.getIntSamplingType(
                 )) + ")")
        logPrint("Sampling Categories = " + str(
            self.samplingTypeInstanceTrain.getStringsPerCategoryToSample()))
        logPrint("Percent of Samples to extract per Sampling Category = " + str(
            self.samplingTypeInstanceTrain.getPercentOfSamplesPerCategoryToSample(
            )))
        logPrint(
            "Provided Weight-Maps, pointing where to focus sampling for each category (if False, samples will be extracted based on GT and ROI) = "
        ...
        logPrint("Number of Subepochs per epoch = " + str(
        ...
        logPrint(
            "Number of Segments loaded on GPU per subepoch for Training = " +
            str(self.segmentsLoadedOnGpuPerSubepochTrain) +
            ". NOTE: This number of segments divided by the batch-size defines the number of optimization-iterations that will be performed every subepoch!")

        logPrint("~~Learning Rate Schedule~~")
        logPrint(
            "Schedule Type: Stable (0), Auto (1), Predefined (2), Exponential (3) = "
            + str(self.stable0orAuto1orPredefined2orExponential3LrSchedule))
        logPrint(
            "[Stable/Auto/Predefined] When decreasing Learning Rate, divide LR by = "
        ...
        logPrint(
            "[Predefined] Predefined Schedule of Epochs when the LR will be lowered = "
            + str(self.predefinedSchedLowerLrAtEpochs))
        logPrint(
            "[Exponential] [Number of initial epochs to not change LR and Mom, final value of LR, final value of Momentum] = "
            + str(self.exponentialSchedForLrAndMom))

        logPrint("~~Data Augmentation During Training~~")
        logPrint("Reflect images per axis = " + str(self.reflectImagesPerAxis))
        logPrint("Perform intensity-augmentation [I'= (I+shift)*mult] = " +
                 str(self.performIntAugm))
        logPrint("[Int. Augm.] Sample Shift from N(mu,std) = " + str(
            self.intAugmOnPairs2Images1None0_imAlreadyNormal_ShiftMuStd_MultiMuStd[
                2]))
        logPrint("[Int. Augm.] Sample Multi from N(mu,std) = " + str(
            self.intAugmOnPairs2Images1None0_imAlreadyNormal_ShiftMuStd_MultiMuStd[
                3]))
        logPrint(
            "[Int. Augm.] (DEBUGGING:) full parameters [no(0)/images(1)/segms(2), nonNorm(0)/norm(1), shift, mult] = "
            + str(
                self.
                intAugmOnPairs2Images1None0_imAlreadyNormal_ShiftMuStd_MultiMuStd))

        logPrint("~~~~~~~~~~~~~~~~~~Validation parameters~~~~~~~~~~~~~~~~")
        logPrint("Perform Validation on Samples throughout training? = " + str(
        ...
        logPrint(
            "Filepaths to Channels of the Validation Cases (Req for either of the above) = "
            + str(self.channelsFilepathsVal))
        logPrint(
            "Provided Ground-Truth for Validation = " + str(
                self.providedGtVal) +
            ". NOTE: Required for Val on samples. Not Req for Full-Inference, but DSC will be reported if provided.")
        logPrint("Filepaths to Ground-Truth labels of the Validation Cases = "
                 + str(self.gtLabelsFilepathsVal))
        logPrint(
            "Provided ROI masks for Validation = " + str(
                self.providedRoiMasksVal) +
            ". NOTE: Validation-sampling and Full-Inference will be limited within this mask if provided. If not provided, Negative Validation samples will be extracted from whole volume, except if advanced-sampling is enabled, and the user provided separate weight-maps for sampling.")
        logPrint("Filepaths to ROI masks for Validation Cases = " + str(
            self.roiMasksFilepathsVal))

        logPrint("~~~~~~~Validation on Samples throughout Training~~~~~~~")
        logPrint(
            "Number of Segments loaded on GPU per subepoch for Validation = " +
            str(self.segmentsLoadedOnGpuPerSubepochVal))

        logPrint("~~Advanced Sampling~~")
        logPrint(
            "Using default uniform sampling for validation = " + str(
                self.useDefaultUniformValidationSampling) +
            ". NOTE: Adv.Sampl.Params are auto-set to perform uniform-sampling if True.")
        logPrint("Type of Sampling = " + str(
            self.samplingTypeInstanceVal.getStringOfSamplingType()) + " (" +
                 str(self.samplingTypeInstanceVal.getIntSamplingType()) + ")")
        logPrint("Sampling Categories = " + str(
            self.samplingTypeInstanceVal.getStringsPerCategoryToSample()))
        logPrint("Percent of Samples to extract per Sampling Category = " + str(
            self.samplingTypeInstanceVal.getPercentOfSamplesPerCategoryToSample(
            )))
        logPrint(
            "Provided Weight-Maps, pointing where to focus sampling for each category (if False, samples will be extracted based on GT and ROI) = "
        ...
        logPrint("Save Probability Maps for each class = " + str(
            self.saveProbMapsBoolPerClassVal))
        logPrint("Filepaths to save results per case = " + str(
            self.filepathsToSavePredictionsForEachPatientVal))
        logPrint("~~Feature Maps~~")
        logPrint("Save Feature Maps = " + str(self.saveIndividualFmImagesVal))
        logPrint("Save FMs in a 4D-image = " + str(
            self.saveMultidimensionalImageWithAllFmsVal))
        logPrint(
            "Min/Max Indices of FMs to visualise per pathway-type and per layer = "
            + str(self.indicesOfFmsToVisualisePerPathwayAndLayerVal))
        logPrint("Filepaths to save FMs per case = " + str(
            self.filepathsToSaveFeaturesForEachPatientVal))

        logPrint("~~Optimization~~")
        logPrint("Initial Learning rate = " + str(self.learningRate))
        logPrint("Optimizer to use: SGD(0), Adam(1), RmsProp(2) = " + str(
            self.optimizerSgd0Adam1Rms2))
        logPrint("Parameters for Adam: b1= " + str(self.b1Adam) + ", b2=" +
                 str(self.b2Adam) + ", e= " + str(self.eAdam))
        logPrint("Parameters for RmsProp: rho= " + str(self.rhoRms) + ", e= " +
                 str(self.eRms))
        logPrint("Momentum Type: Classic (0) or Nesterov (1) = " + str(
            self.classicMom0Nesterov1))
        logPrint("Momentum Non-Normalized (0) or Normalized (1) = " + str(
        ...
        logPrint("Normal pathway's layers to freeze = " + str(
            self.indicesOfLayersPerPathwayTypeToFreeze[0]))
        logPrint("Subsampled pathway's layers to freeze = " + str(
            self.indicesOfLayersPerPathwayTypeToFreeze[1]))
        logPrint("FC pathway's layers to freeze = " + str(
            self.indicesOfLayersPerPathwayTypeToFreeze[2]))
    ...
    def getTupleForCnnTraining(self):
        ...
        trainTuple = (
            ...
            self.gtLabelsFilepathsVal,
            self.
            providedWeightMapsToSampleForEachCategoryTraining,  #Always true, since either GT labels or advanced-mask-where-to-pos
            self.
            forEachSamplingCategory_aListOfFilepathsToWeightMapsOfEachPatientTraining,
            self.
            providedWeightMapsToSampleForEachCategoryValidation,  #If false, corresponding samples will be extracted uniformly from whole image.
            self.
            forEachSamplingCategory_aListOfFilepathsToWeightMapsOfEachPatientValidation,
            ...
            self.
            roiMasksFilepathsVal,  # also used for fast inf and also for uniform sampling of segs.
            borrowFlag,
            self.numberOfEpochs,
            self.numberOfSubepochs,
            ...
            self.
            performFullInferenceOnValidationImagesEveryFewEpochs,  #Even if not providedGtForValidationBool, inference will be performed if this == True, to save the results, eg for visual.
            self.
            numberOfEpochsBetweenFullInferenceOnValImages,  # Should not be == 0, except if performFullInferenceOnValidationImagesEveryFewEpochsBool == False
