...
def makeFoldersToSaveFilesForThisRun():
    ...
    parentFolderForSavingPredictedImages = folderToSavePredictionImages[:folderToSavePredictionImages.find(
        "/")]  #Usually should be ./predictions"
    if not os.path.exists(parentFolderForSavingPredictedImages):
        os.mkdir(parentFolderForSavingPredictedImages)
    if not os.path.exists(
            folderToSavePredictionImages):  #The inner folder, for this particular run.
...
def makeFoldersNeededForTrainingSession(absMainOutputFolder, sessionName):
    ...
    return [folderForSessionCnnModels, folderForLogs, folderForSegmAndProbMaps,
            folderForFeatures]
...
def makeFoldersNeededForCreateModelSession(absMainOutputFolder, modelName):
    ...
    return [folderForCnnModels, folderForLogs]
