...
class TestSessionParameters(object):
    ...
    def __init__(self,
        ...
        self.saveProbMapsBoolPerClass = saveProbMapsBoolPerClass if (
            saveProbMapsBoolPerClass <> None and saveProbMapsBoolPerClass <>
        ...
        self.indicesOfFmsToVisualisePerPathwayAndLayer = [
            ...
            for item in indicesOfFmsToVisualisePerPathwayAndLayer
        ]
    ...
    def _makeFilepathsForPredictionsAndFeatures(
        ...
        if self.namesToSavePredictionsAndFeatures <> None:
            for case_i in xrange(self.numberOfCases):
                filepathForCasePrediction = absPathToFolderForPredictionsFromSession + "/" + self.namesToSavePredictionsAndFeatures[
                    case_i]
                self.filepathsToSavePredictionsForEachPatient.append(
                    filepathForCasePrediction)
                filepathForCaseFeatures = absPathToFolderForFeaturesFromSession + "/" + self.namesToSavePredictionsAndFeatures[
                    case_i]
                self.filepathsToSaveFeaturesForEachPatient.append(
    ...
    def printParametersOfThisSession(self):
        ...
        logPrint("~~~~~~~~~~~~~~~~~~~~INPUT~~~~~~~~~~~~~~~~")
        logPrint("Number of Classes (from CNN Model) = " + str(
            self.numberOfClasses))
        logPrint("Number of cases to perform inference on = " + str(
            self.numberOfCases))
        logPrint("number of channels (modalities) = " + str(
            self.numberOfChannels))
        logPrint("Paths to the channels of each case = " + str(
            self.channelsFilepaths))
        logPrint("User provided Ground Truth labels for DSC calculation = " +
                 str(self.providedGt))
        if not self.providedGt:
            logPrint(
                ">>> WARN: The DSC accuracy will NOT be evaluated and reported!")
        logPrint("Paths to the provided GT labels per case = " + str(
            self.gtLabelsFilepaths))
        logPrint(
            "User provided Region-Of-Interest Masks for faster inference = " +
            str(self.providedRoiMasks))
        logPrint("Filepaths of the ROI Masks provided per case = " + str(
            self.roiMasksFilepaths))
        if not self.providedRoiMasks:
            logPrint(
                ">>> WARN: Inference will be performed on whole scan. Consider providing a ROI image for faster results, if possible!")

        logPrint("~~~~~~~~~~~~~~~~~~~OUTPUT~~~~~~~~~~~~~~~")
        logPrint("Path to the main output-folder = " + str(
            self.mainOutputAbsFolder))
        logPrint("Provided names to use to save results for each case = " +
                 str(self.namesToSavePredictionsAndFeatures))

        logPrint(
            "~~~~~~~Ouput-parameters for Predictions (segmentation and probability maps)~~~~")
        logPrint("Save the predicted segmentation = " + str(
            self.saveSegmentation))
        logPrint("Save the probability maps = " + str(
            self.saveProbMapsBoolPerClass))
        logPrint("Paths where to save predictions per case = " + str(
            self.filepathsToSavePredictionsForEachPatient))
        ...
        logPrint("Save FMs in individual images = " + str(
            self.saveIndividualFmImages))
        logPrint("Save all requested FMs in one 4D image = " + str(
            self.saveMultidimensionalImageWithAllFms))
        if self.saveMultidimensionalImageWithAllFms:
            logPrint(
                ">>> WARN : The 4D image can be hundreds of MBytes if the CNN is big and many FMs are chosen to be saved. Configure wisely.")
        logPrint(
        ...
        logPrint("Save Feature Maps at = " + str(
            self.filepathsToSaveFeaturesForEachPatient))
        ...
        if not self.padInputImagesBool:
            logPrint(
                ">>> WARN: Inference near the borders of the image might be incomplete if not padded! Although some speed is gained if not padded. Task-specific, your choice.")
        logPrint(
