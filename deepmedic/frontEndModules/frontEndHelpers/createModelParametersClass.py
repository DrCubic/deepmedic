...
from deepmedic.cnnHelpers import (
    ...
    checkKernDimPerLayerCorrect3dAndNumLayers,
    checkReceptiveFieldFineInComparisonToSegmentSize, checkSubsampleFactorEven)
...
class CreateModelSessionParameters(object):
    ...
    def checkLayersForResidualsGivenDoNotInclude1st(
            self, residConnAtLayersNormal, residConnAtLayersSubsampled,
            residConnAtLayersFc):
    ...
    #ERRORS
    @staticmethod
    def errorSegmDimensionsSmallerThanReceptiveF(
            receptiveFieldNormal, segmentDimensions, tr0_val1_inf2):
        stringsTrainValInference = ["TRAINING", "VALIDATION", "INFERENCE"]
        print "ERROR: The segment-size (input) should be at least as big as the receptive field of the model! The network was made with a receptive field of dimensions: ", receptiveFieldNormal, ". But in the case of :", stringsTrainValInference[
            tr0_val1_inf2], " the dimensions of the input segment were specified smaller: ", segmentDimensions, ". Please fix this by adjusting number of layer and kernel dimensions! Exiting!"
        exit(1)

    @staticmethod
    def errorRequireNumberOfClasses():
        print "ERROR: Number of classses not specified in the config file, which is required. Please specify in the format: numberOfOutputClasses = 3 (any integer). This number should be including the background class! For instance if the class is binary, set this to 2! Exiting!"
        exit(1)
    ...
    def errorRequireNumberOfChannels():
        print "ERROR: Parameter \"numberOfInputChannels\" not specified or specified smaller than 1. Please specify the number of input channels that will be used as input to the CNN, in the format: numberOfInputChannels = number (an integer > 0). Exiting!"
        exit(1)
    ...
    def errorRequireKernelDimensionsSubsampled(numFMsPerLayerNormal,
        ...
        print "ERROR: It was requested to use the 2-scale architecture, with a subsampled pathway. Because of limitations to the developed system, the two pathways must have the save size of receptive field. By default, if \"useSubsampledPathway\" = True, and the parameters \"numberFMsPerLayerSubsampled\" and \"kernelDimPerLayerSubsampled\" are not specified, the second pathway will be constructed symmetrical to the first. However, in this case, \"numberFMsPerLayerSubsampled\" was specified. It was found to have ", len(
            numFMsPerLayerSubsampled), " entries, which specified this amount of layers in the subsampled pathway. This is different than the number of layers in the Normal pathway, specified to be: ", len(
                numFMsPerLayerNormal), ". In this case, we require you to also provide the parameter \"numberFMsPerLayerSubsampled\", specifying kernel dimensions in the subsampled pathway, in a fashion that results in same size of receptive field as the normal pathway."
    ...
    @staticmethod
    def errorReceptiveFieldsOfNormalAndSubsampledDifferent(
            kernDimPerLayerNormal, receptiveFieldSubsampled):
        print "ERROR: The receptive field of the normal pathway was calculated = ", len(
            kernDimPerLayerNormal), " while the receptive field of the subsampled pathway was calculated=", len(
                receptiveFieldSubsampled), ". Because of limitations to the developed system, the two pathways must have the save size of receptive field. Please provide a combination of \"numberFMsPerLayerSubsampled\" and \"kernelDimPerLayerSubsampled\" that gives the same size of field as the normal pathway. If unsure of how to proceed, please ommit specifying \"numberFMsPerLayerSubsampled\" and \"kernelDimPerLayerSubsampled\" in the config file, and the second subsampled pathway will be automatically created to mirror the normal. Else, if you want to just specify the number of Feature Maps in the subsampled, provide \"numberFMsPerLayerSubsampled\" = [num-FMs-layer1, ..., num-FMs-layerN], with N the same number as the normal pathway, and we will then use the same kernel-sizes as the normal pathway. Exiting!"
        exit(1)

    @staticmethod
    def errorReqInitializationMethod01():
        print "ERROR: Parameter \"initializeClassic0orDelving1\" must be given equal to 0 or 1. Omit for default (=1). Exiting!"
        exit(1)

    @staticmethod
    def errorReqActivFunction01():
        print "ERROR: Parameter \"relu0orPrelu1\" must be given equal to 0 or 1. Omit for default (=1). Exiting!"
        exit(1)
    ...
    def errorResLayer1(strPathwayType):
        ...
        print "\t Provide a list that does not iinclude the first layer, eg layersWithResidualConnNormal = [4,6,8], or an empty list [] for no such connections. Exiting!"
        exit(1)
    ...
    def warnSubFactorOdd():
        ...
        # OTHERS
    @staticmethod
    def changeDatastructureToListOfListsForSecondaryPathwaysIfNeeded(
            listFromConfig):
        # subsampleFactorFromConfig: whatever given in the config by the user (except None).
        if not isinstance(listFromConfig, list):
            print "ERROR: variable \"", listFromConfig, "\" given in modelConfig.cfg should be either a list of integers, or a list of lists of integers, in case multiple lower-scale pathways are wanted. Please correct it. Exiting."
            exit(1)
        ...
        if not (allElementsAreLists or
                noElementIsList):  #some are lists and some are not
            print "ERROR: variable \"", listFromConfig, "\" given in modelConfig.cfg should be either a list of integers, or a list of lists of integers, in case multiple lower-scale pathways are wanted. Please correct it. Exiting."
            exit(1)
    ...
    def __init__(self,
        ...
        if not self.useSubsampledBool:
            ...
            if kernDimSubsampled == None and numOfLayersInEachSubPath == len(
                ...
                self.receptiveFieldSubsampled = self.receptiveFieldNormal
                    ...
                    self.
                    numFMsPerLayerNormal):  #user specified subsampled layers.
            ...
            self.subsampleFactor = self.changeDatastructureToListOfListsForSecondaryPathwaysIfNeeded(
                self.subsampleFactor)
            for secondaryPathway_i in xrange(len(
                    self.
                    subsampleFactor)):  #It should now be a list of lists, one sublist per secondary pathway. This is what is currently defining how many pathways to use.
            ...
            numOfSubsPaths = max(
                len(self.numFMsPerLayerSubsampled), len(self.subsampleFactor))
            # Default behaviour:
            ...
            for _ in range(numOfSubsPaths - len(
                    self.numFMsPerLayerSubsampled)):
        ...
        assert len(self.kernelDimensionsFirstFcLayer) == 3 and (
            False not in [dim > 0
                          for dim in self.kernelDimensionsFirstFcLayer])
        ...
        for (tr0_val1_inf2, segmentDimensions) in [
            (0, self.segmDimNormalTrain), (1, self.segmDimNormalVal),
        ...
        self.dropoutRatesForAllPathways = [self.dropNormal,
                                           self.dropSubsampled, self.dropFc,
                                           []]
        ...
        self.ranksOfLowerRankLayersForEachPathway = [
            ...
            [2
             for layer_i in self.indicesOfLowerRankLayersPerPathway[1]], [], []
        ]
        #============= HIDDENS ======================
        ...
        self.maxPoolingParamsStructure = [mpParamsNorm, mpParamsSubs,
                                          mpParamsFc]
    ...
    def printParametersOfThisSession(self):
        ...
        logPrint("Path and filename to save model = " + str(
            self.pathAndFilenameToSaveModel))

        logPrint("~~~~~~~~~~~~~~~~~~Model parameters~~~~~~~~~~~~~~~~")
        logPrint("Number of Classes (including background) = " + str(
            self.numberClasses))
        logPrint("~~Normal Pathway~~")
        logPrint("Number of Input Channels = " + str(
            self.numberOfInputChannelsNormal))
        logPrint("Number of Layers = " + str(len(self.numFMsPerLayerNormal)))
        logPrint("Number of Feature Maps per layer = " + str(
            self.numFMsPerLayerNormal))
        logPrint("Kernel Dimensions per layer = " + str(
        ...
        logPrint("Lower Rank layers will be made of rank = " + str(
            self.ranksOfLowerRankLayersForEachPathway[0]))
        ...
        logPrint("Number of subsampled pathways that will be built = " + str(
            len(self.subsampleFactor)))
        logPrint("Number of Layers (per sub-pathway) = " + str(
            [len(numFmsPerLayerForSubPath)
             for numFmsPerLayerForSubPath in self.numFMsPerLayerSubsampled]))
        logPrint("Number of Feature Maps per layer (per sub-pathway) = " + str(
            self.numFMsPerLayerSubsampled))
        logPrint("Kernel Dimensions per layer = " + str(
            self.kernDimPerLayerSubsampled))
        logPrint("Receptive Field = " + str(self.receptiveFieldSubsampled))
        logPrint("Subsampling Factor per dimension (per sub-pathway) = " + str(
            self.subsampleFactor))
        logPrint(
            "Residual connections added at the output of layers (indices from 0) = "
        ...
        logPrint("Lower Rank layers will be made of rank = " + str(
            self.ranksOfLowerRankLayersForEachPathway[1]))
        ...
        logPrint(
            ...
            + str(len(self.numFMsInExtraFcs)))
        logPrint("Number of Feature Maps in the additional FC layers = " + str(
        ...
        logPrint(
            "Dimensions of Kernels in the 1st FC layer (Classif. layer if no hidden FCs used) = "
            + str(self.kernelDimensionsFirstFcLayer))

        logPrint("~~Size Of Image Segments~~")
        logPrint("Size of Segments for Training = " + str(
            self.segmDimNormalTrain))
        logPrint("Size of Segments for Validation = " + str(
            self.segmDimNormalVal))
        logPrint("Size of Segments for Testing = " + str(
        ...
        logPrint("Drop.R. for each layer in Normal Pathway = " + str(
            self.dropoutRatesForAllPathways[0]))
        logPrint("Drop.R. for each layer in Subsampled Pathway = " + str(
            self.dropoutRatesForAllPathways[1]))
        logPrint(
        ...
        logPrint(
            "Classic random N(0,0.01) initialization (0), or ala \"Delving Into Rectifier\" (1) = "
            + str(self.initialMethodClassic0Delving1))

        logPrint("~~Activation Function~~")
        logPrint("ReLU (0), or PReLU (1) = " + str(
    ...
    def getTupleForCnnCreation(self):
        ...
        cnnCreationTuple = (
            ...
            #=== Normal Pathway ===
            self.
            numFMsPerLayerNormal,  #ONLY for the convolutional layers, NOT the final convFCSoftmaxLayer!
            self.kernDimPerLayerNormal,
            #=== Subsampled Pathway ===
            self.numFMsPerLayerSubsampled,
            self.kernDimPerLayerSubsampled,
