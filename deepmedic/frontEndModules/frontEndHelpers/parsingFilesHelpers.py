...
def getAbsPathEvenIfRelativeIsGiven(pathGiven,
    ...
    if os.path.isabs(pathGiven):
        ...
        if os.path.isdir(absolutePathToWhereRelativePathRelatesTo):
            ...
            relativePathToWhatGiven = os.path.dirname(
                absolutePathToWhereRelativePathRelatesTo)
...
def parseAbsFileLinesInList(pathToListingFile):
    ...
    with open(pathToListingFile, "r") as inp:
        for line in inp:
            if line.strip(
            ) == "-":  # symbol indicating the non existence of this channel. Will be zero-filled.
                list1.append("-")
                ...
                if os.path.isabs(pathToFileParsed):  #abs path.
                    ...
                    list1.append(
                        os.path.normpath(pathToFolderContainingThisListFile +
                                         "/" + pathToFileParsed))
    return list1


def checkListContainsCorrectNumberOfCasesOtherwiseExitWithError(
        numberOfCasesPreviously, pathToGivenListFile,
...
def checkThatAllEntriesOfAListFollowNameConventions(
