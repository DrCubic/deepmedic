...
from deepmedic.frontEndModules.frontEndHelpers.parsingFilesHelpers import (
    checkIfAllElementsOfAListAreFilesAndExitIfNot,
    checkListContainsCorrectNumberOfCasesOtherwiseExitWithError,
    checkThatAllEntriesOfAListFollowNameConventions,
    getAbsPathEvenIfRelativeIsGiven, parseAbsFileLinesInList,
    parseFileLinesInList)
from deepmedic.frontEndModules.frontEndHelpers.preparationForSessionHelpers import \
    makeFoldersNeededForTestingSession
from deepmedic.frontEndModules.frontEndHelpers.testParametersClass import \
    TestSessionParameters
from deepmedic.genericHelpers import load_object_from_gzip_file
from deepmedic.trainValidateTestVisualiseParallel import \
...
class TestConfig(object):
    ...
    def get(self, string1):
        return self.configStruct[
            string1] if string1 in self.configStruct else None
    ...
    def checkIfConfigIsCorrectForParticularCnnModel(self, cnnInstance):
        ...
        if len(self.configStruct[
                self.CHANNELS]) <> cnnInstance.numberOfImageChannelsPath1:
            print "ERROR:\tConfiguration parameter \"", self.configStruct[
                self.
                CHANNELS], "\" should have the same number of elements as the number of channels specified when constructing the cnnModel!\n\tCnnModel was constructed to take as input #", cnnInstance.numberOfImageChannelsPath1, " while the list given in the config-file contained #", len(
                    self.configStruct[self.CHANNELS]), " elements."
        ...
        if self.configStruct[self.SAVE_PROBMAPS_PER_CLASS] and len(
                self.configStruct[
                    self.
                    SAVE_PROBMAPS_PER_CLASS]) <> cnnInstance.numberOfOutputClasses:
            print "ERROR:\tConfiguration parameter \"", self.configStruct[
                self.
                SAVE_PROBMAPS_PER_CLASS], "\" should have the same number of elements as the number of classes in the task! CnnModel was constructed to predict #", cnnInstance.numberOfOutputClasses, " while the list for the parameter contained #", len(
                    self.configStruct[
                        self.SAVE_PROBMAPS_PER_CLASS]), " elements."
            print "ERROR:\tConfiguration parameter \"", self.configStruct[
                self.
                SAVE_PROBMAPS_PER_CLASS], "\" should be a list of booleans, one for each class of the task (including the background as class-0). True to save the predicted probability map for the corresponding class, False otherwise.\n\tAs an example, it should be given in the form: ", self.configStruct[
                    self.
                    SAVE_PROBMAPS_PER_CLASS], " = [False, True, False] (python style boolean list).\n\tThis would result in not saving the prob-maps for the class-0 (background), save for class-1, and not save for class-2. Please correct it or ommit it completely for default.\nExiting!"
            exit(1)
        ...
        savingFms = self.configStruct[
            self.SAVE_INDIV_FMS] or self.configStruct[self.SAVE_4DIM_FMS]
        numNormLayers = len(cnnInstance.pathways[
            cnnInstance.CNN_PATHWAY_NORMAL].getLayers())
        numSubsLayers = len(cnnInstance.pathways[
            cnnInstance.CNN_PATHWAY_SUBSAMPLED].getLayers())
        numFcLayers = len(cnnInstance.pathways[
            cnnInstance.CNN_PATHWAY_FC].getLayers())
        numLayerEntriesGivenNorm = None if not self.configStruct[
            self.INDICES_OF_FMS_TO_SAVE_NORMAL] else len(self.configStruct[
                self.INDICES_OF_FMS_TO_SAVE_NORMAL])
        numLayerEntriesGivenSubs = None if not self.configStruct[
            self.INDICES_OF_FMS_TO_SAVE_SUBSAMPLED] else len(self.configStruct[
                self.INDICES_OF_FMS_TO_SAVE_SUBSAMPLED])
        numLayerEntriesGivenFc = None if not self.configStruct[
            self.INDICES_OF_FMS_TO_SAVE_FC] else len(self.configStruct[
                self.INDICES_OF_FMS_TO_SAVE_FC])

        wrongFmsEntryForNormal = True if (
            savingFms and numLayerEntriesGivenNorm <> None and
            numLayerEntriesGivenNorm <> numNormLayers) else False
        wrongFmsEntryForSubsampled = True if (
            savingFms and numLayerEntriesGivenSubs <> None and
            numLayerEntriesGivenSubs <> numSubsLayers) else False
        wrongFmsEntryForFc = True if (
            savingFms and numLayerEntriesGivenFc <> None and
        ...
        if wrongFmsEntryForNormal:
            print "\tNormal pathway: In config file: ", self.configStruct[
                self.
                INDICES_OF_FMS_TO_SAVE_NORMAL], " given #", numLayerEntriesGivenNorm, " elements, while the CNN-model has #", numNormLayers, " layers in this pathway."
        if wrongFmsEntryForSubsampled:
            print "\tSubsampled pathway: In config file: ", self.configStruct[
                self.
                INDICES_OF_FMS_TO_SAVE_SUBSAMPLED], " given #", numLayerEntriesGivenSubs, " elements, while the CNN-model has #", numSubsLayers, " layers in this pathway."
        if wrongFmsEntryForFc:
            print "\tFully-Connected pathway: In config file: ", self.configStruct[
                self.
                INDICES_OF_FMS_TO_SAVE_FC], " given #", numLayerEntriesGivenFc, " elements, while the CNN-model has #", numFcLayers, " layers in this pathway."
        if wrongEntryGiven:
            print "\tExiting!"
            exit(1)
...
def checkIfMainTestConfigIsCorrect(testConfig, testConfigFilepath,
    ...
    if not configStruct[testConfig.FOLDER_FOR_OUTPUT] or not isinstance(
        ...
        exit(1)
    if not (configStruct[testConfig.CNN_MODEL_FILEPATH] and
            isinstance(configStruct[testConfig.CNN_MODEL_FILEPATH],
    ...
    if not configStruct[testConfig.CHANNELS] or not isinstance(
            configStruct[testConfig.CHANNELS], list) or not all(isinstance(
                item, str) for item in configStruct[testConfig.CHANNELS]):
...
def checkIfFilesThatListFilesPerCaseAreCorrect(testConfig, testConfigFilepath):
    ...
    listOfListingFilesProvided = listOfListingFilesProvided if not configStruct[
        testConfig.ROI_MASKS] else listOfListingFilesProvided + [
            configStruct[testConfig.ROI_MASKS]
        ]
    listOfListingFilesProvided = listOfListingFilesProvided if not configStruct[
        testConfig.GT_LABELS] else listOfListingFilesProvided + [
            configStruct[testConfig.GT_LABELS]
        ]
    for pathToListingFile_i in xrange(len(listOfListingFilesProvided)):
        pathToCurrentFileListingPaths = listOfListingFilesProvided[
            pathToListingFile_i]
        absolutePathToCurrentFileListingPaths = getAbsPathEvenIfRelativeIsGiven(
            pathToCurrentFileListingPaths, testConfigFilepath)
        if not os.path.isfile(absolutePathToCurrentFileListingPaths):
            ...
            print "Exiting!"
            exit(1)
        listOfFilepathsForEachCaseInCurrentListingFile = parseAbsFileLinesInList(
        ...
        if numberOfCases == -1:
            ...
            checkListContainsCorrectNumberOfCasesOtherwiseExitWithError(
                numberOfCases, absolutePathToCurrentFileListingPaths,
    ...
    listingFileWithNamesForPredictionsPerCase = configStruct[
        testConfig.NAMES_FOR_PRED_PER_CASE]
    absolutePathToListingFileWithPredictionNamesPerCase = getAbsPathEvenIfRelativeIsGiven(
        listingFileWithNamesForPredictionsPerCase, testConfigFilepath)
    if not os.path.isfile(absolutePathToListingFileWithPredictionNamesPerCase):
        ...
        print "Exiting!"
        exit(1)
    listOfPredictionNamesForEachCaseInListingFile = parseFileLinesInList(
        absolutePathToListingFileWithPredictionNamesPerCase)  #CAREFUL: Here we use a different parsing function!
    checkThatAllEntriesOfAListFollowNameConventions(
...
def checkIfOptionalParametersAreGivenCorrectly(testConfig, testConfigFilepath):
    ...
    if configStruct[testConfig.SESSION_NAME] and not isinstance(
            configStruct[testConfig.SESSION_NAME], str):
        print "ERROR: Optional parameter ", configStruct[
            testConfig.
            SESSION_NAME], " should be a string, given in the format: ", configStruct[
                testConfig.
                SESSION_NAME], " = \"yourTestSessionName\".\nExiting!"
        exit(1)

    listOfBooleanParametersStrings = [testConfig.PAD_INPUT,
                                      testConfig.SAVE_SEGM,
                                      testConfig.SAVE_INDIV_FMS,
                                      testConfig.SAVE_4DIM_FMS, ]
    for booleanParameterString in listOfBooleanParametersStrings:
        if (configStruct[booleanParameterString]) and not (
                configStruct[booleanParameterString] in [True, False]):
            print "ERROR: Optional parameter ", configStruct[
                booleanParameterString], " should be given either True or False in the format: ", configStruct[
                    booleanParameterString], " = True/False (No quotes, first letter capital. Python-Boolean style!).\nExiting!"
            exit(1)
    ...
    if (variableSaveProbMaps):
        valid1 = True
        if isinstance(variableSaveProbMaps,
                      list) and len(variableSaveProbMaps) > 0:
    ...
    listOfTheIndicesOfFmsPerPathwayType = [
        testConfig.INDICES_OF_FMS_TO_SAVE_NORMAL,
        testConfig.INDICES_OF_FMS_TO_SAVE_SUBSAMPLED,
        testConfig.INDICES_OF_FMS_TO_SAVE_FC
    ]
    for indicesOfFmsPerPathwayTypeString in listOfTheIndicesOfFmsPerPathwayType:
        if (configStruct[indicesOfFmsPerPathwayTypeString]):
            variableIndicesOfFmsPerPathwayType = configStruct[
                indicesOfFmsPerPathwayTypeString]
            valid1 = True
            if not isinstance(variableIndicesOfFmsPerPathwayType, list):
                valid1 = False
            elif len(
                    variableIndicesOfFmsPerPathwayType) > 0:  #Remember, it can be [] to save no feature maps from this pathway!
                    ...
                    if isinstance(entryForALayer, list):
                        if len(entryForALayer) > 0:  #This can be [], to not save any FM of this layer.
                            for indexOfFm in entryForALayer:
                                if not (isinstance(indexOfFm, int) and
                                        indexOfFm >= 0):
                                    valid1 = False
                    else:
                        valid1 = False
...
def deepMedicTestMain(testConfigFilepath, absPathToSavedModelFromCmdLine):
    ...
    sessionName = configGet(testConfig.SESSION_NAME) if configGet(
        testConfig.
        SESSION_NAME) else TestSessionParameters.getDefaultSessionName()
    ...
    sessionLogger.print3("The CNN model was loaded successfully from: " + str(
        filepathToCnnModelToLoad))
    ...
    listOfAListPerChannelWithFilepathsOfAllCases = [
        parseAbsFileLinesInList(getAbsPathEvenIfRelativeIsGiven(
            channelConfPath, testConfigFilepath))
    ...
    gtLabelsFilepaths = parseAbsFileLinesInList(
        getAbsPathEvenIfRelativeIsGiven(
            configGet(testConfig.GT_LABELS), testConfigFilepath)) if configGet(
                testConfig.GT_LABELS) else None
    roiMasksFilepaths = parseAbsFileLinesInList(
        getAbsPathEvenIfRelativeIsGiven(
            configGet(testConfig.ROI_MASKS), testConfigFilepath)) if configGet(
    ...
    namesToSavePredsAndFeats = parseFileLinesInList(
        getAbsPathEvenIfRelativeIsGiven(
            ...
            testConfigFilepath)) if configGet(
                testConfig.
                NAMES_FOR_PRED_PER_CASE) else None  #CAREFUL: Here we use a different parsing function!
