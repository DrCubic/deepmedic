...
class SamplingType(object):
    def __init__(self, myLogger, samplingType, numberOfClassesInclBackgr):
        ...
        if self.samplingType == 0:
            ...
            self.arrayWithStringPerCategoryToSample = ["Foreground",
                                                       "Background"]
            ...
            self.arrayWithStringPerCategoryToSample = [
                "Class-" + str(i) for i in xrange(numberOfClassesInclBackgr)
            ]
        else:
            self.logger.print3(
                "ERROR: Tried to create a SamplingType instance, but the samplingType passed was invalid. Should be [0,1,2,3]. Exiting!")
            exit(1)

    def setPercentOfSamplesPerCategoryToSample(
            self, percentageOfSamplesPerCategoryOfSampling):
        if self.samplingType in [0, 3] and len(
                percentageOfSamplesPerCategoryOfSampling) <> self.getNumberOfCategoriesToSample(
                ):
            self.logger.print3(
                "ERROR: In class [SamplingType], the list percentageOfSamplesPerCategoryOfSampling had ["
                + str(len(percentageOfSamplesPerCategoryOfSampling)) +
                "] elements. In this case of [" + self.stringOfSamplingType +
                "], it should have [" + str(self.getNumberOfCategoriesToSample(
                )) + "]! Exiting!")
            exit(1)

        if self.samplingType == 0:
            self.percentOfSamplesPerCategoryToSample = self.normalizePercentages(
                percentageOfSamplesPerCategoryOfSampling)
        elif self.samplingType == 1:
            ...
            self.logger.print3(
                "ERROR: in [SamplingType]. self.samplingType was invalid. Should be [0,1,2,3]. Exiting!")
            exit(1)
    ...
    def logicDecidingAndGivingFinalSamplingMapsForEachCategory(
            self,
        ...
        if self.samplingType == 0:  # fore/background
            if providedWeightMapsToSampleForEachCategory:  #Both weight maps should be provided currently.
                numOfProvidedWeightMaps = len(
                    arrayWithWeightMapsWhereToSampleForEachCategory)
                if numOfProvidedWeightMaps <> self.getNumberOfCategoriesToSample(
                ):
                    self.logger.print3(
                        "ERROR: For SamplingType = Fore/Background(0), [" +
                        str(numOfProvidedWeightMaps) +
                        "] weight maps were provided! Two (fore/back) were expected! Exiting!")
                    exit(1)
                ...
                self.logger.print3(
                    "ERROR: For SamplingType=[" + self.stringOfSamplingType +
                    "], if weighted-maps are not provided, at least Ground Truth labels should be given to extract foreground! Exiting!")
                exit(1)
            elif providedRoiMaskBool:  # and providedGtLabelsBool
                maskForForegroundSampling = (gtLabelsImage > 0).astype(int)
                maskForBackgroundSampling_roiMinusGtLabels = (roiMask > 0) * (
                    maskForForegroundSampling == 0)
                finalWeightMapsToSampleFromPerCategoryForSubject = [
                    maskForForegroundSampling,
                ...
                maskForForegroundSampling = (gtLabelsImage > 0).astype(int)
                maskForBackgroundSampling_roiMinusGtLabels = np.ones(
                    dimensionsOfImageChannel,
                    dtype="int16") * (maskForForegroundSampling == 0)
                finalWeightMapsToSampleFromPerCategoryForSubject = [
                    maskForForegroundSampling,
                    maskForBackgroundSampling_roiMinusGtLabels
                ]  #Foreground / Background (in sequence)
            ...
            if providedWeightMapsToSampleForEachCategory:
                ...
                finalWeightMapsToSampleFromPerCategoryForSubject = [np.ones(
                    dimensionsOfImageChannel,
                    dtype="int16")]
            ...
            if providedWeightMapsToSampleForEachCategory:
                ...
                if numOfProvidedWeightMaps <> self.getNumberOfCategoriesToSample(
                    ...
                    self.logger.print3(
                        "ERROR: For SamplingType=[" + self.stringOfSamplingType
                        + "], [" + str(numOfProvidedWeightMaps) +
                        "] weight maps were provided! As many as the classes ["
                        + str(self.getNumberOfCategoriesToSample(
                        )) + "] (incl Background) were expected! Exiting!")
                    exit(1)
                ...
                for cat_i in xrange(self.getNumberOfCategoriesToSample(
                    ...
                    finalWeightMapsToSampleFromPerCategoryForSubject.append((
                        gtLabelsImage == cat_i).astype(int))
            else:
                self.logger.print3(
                    "ERROR: For SamplingType=TargettedPerClass(3), either weightMaps for each class or GT labels should be given! Exiting!")
                exit(1)
        else:
            self.logger.print3(
                "ERROR: Sampling-type-number passed in [logicDecidingAndGivingFinalSamplingMapsForEachCategory] was invalid. Should be [0,1,2,3]. Exiting!")
            exit(1)
