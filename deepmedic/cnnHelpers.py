...
class CnnWrapperForSampling(object):
    ...
    def __init__(self, cnn3dInstance):
        ...
        self.batchSizeTrainValTest = [cnn3dInstance.batchSize,
                                      cnn3dInstance.batchSizeValidation,
                                      cnn3dInstance.batchSizeTesting]
        self.finalTargetLayer_outputShapeTrainValTest = [
            cnn3dInstance.finalTargetLayer.outputShapeTrain,
        ...
        for pathway_i in xrange(len(cnn3dInstance.pathways)):
            self.pathways.append(PathwayWrapperForSampling(
                cnn3dInstance.pathways[pathway_i]))

    def getNumPathwaysThatRequireInput(self):
        return self._numPathwaysThatRequireInput


def dump_cnn_to_gzip_file_dotSave(cnnInstance,
                                  filenameWithPathToSaveTo,
                                  logger=None):
    filenameWithPathToSaveToDotSave = os.path.abspath(filenameWithPathToSaveTo
                                                      + ".save")
    cnnInstance.freeGpuTrainingData()
    cnnInstance.freeGpuValidationData()
    cnnInstance.freeGpuTestingData()
    # Clear out the compiled functions, so that they are not saved with the instance:
    compiledFunctionTrain = cnnInstance.cnnTrainModel
    cnnInstance.cnnTrainModel = ""
    compiledFunctionVal = cnnInstance.cnnValidateModel
    cnnInstance.cnnValidateModel = ""
    compiledFunctionTest = cnnInstance.cnnTestModel
    cnnInstance.cnnTestModel = ""
    compiledFunctionVisualise = cnnInstance.cnnVisualiseFmFunction
    cnnInstance.cnnVisualiseFmFunction = ""

    if logger <> None:
        logger.print3("Saving network to: " + str(
            filenameWithPathToSaveToDotSave))
    else:
...
def calculateSubsampledImagePartDimensionsFromImagePartSizePatchSizeAndSubsampleFactor(
        imagePartDimensions, patchDimensions, subsampleFactor):
    """
    ...
    for rcz_i in xrange(len(imagePartDimensions)):
        centralVoxelsInThisDimension = imagePartDimensions[
            rcz_i] - patchDimensions[rcz_i] + 1
        centralVoxelsInThisDimensionForSubsampledPart = int(ceil(
            centralVoxelsInThisDimension * 1.0 / subsampleFactor[rcz_i]))
        sizeOfSubsampledImagePartInThisDimension = patchDimensions[
            rcz_i] + centralVoxelsInThisDimensionForSubsampledPart - 1
        subsampledImagePartDimensions.append(
            sizeOfSubsampledImagePartInThisDimension)
    return subsampledImagePartDimensions


def calculateReceptiveFieldDimensionsFromKernelsDimListPerLayerForFullyConvCnnWithStrides1(
        kernDimPerLayerList):
    ...
    for dimension_i in xrange(numberOfDimensions):
        for layer_i in xrange(len(kernDimPerLayerList)):
            receptiveField[dimension_i] += kernDimPerLayerList[layer_i][
                dimension_i] - 1
    return receptiveField


def checkReceptiveFieldFineInComparisonToSegmentSize(receptiveFieldDim,
                                                     segmentDim):
...
def checkKernDimPerLayerCorrect3dAndNumLayers(kernDimensionsPerLayer,
                                              numOfLayers):
    ...
    if kernDimensionsPerLayer == None or len(
            kernDimensionsPerLayer) <> numOfLayers:
        return False
