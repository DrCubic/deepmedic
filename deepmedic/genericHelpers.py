...
def strFlXfNA(floatOrNotAppl, numDec, notApplicPattern):
    stringThatSaysHowManyDecimals = "{:." + str(numDec) + "f}"
    return stringThatSaysHowManyDecimals.format(
        floatOrNotAppl) if floatOrNotAppl <> notApplicPattern else notApplicPattern
...
def strListFlXfNA(
        listWithFloatsOrNotAppl, numDec, notApplicPattern
):  # If you just do a normal list, the internal string-floats are printed with quotes ' '
    stringThatSaysHowManyDecimals = "%." + str(numDec) + "f"
    strList = "[ "
...
def strListFl4fNA(
        listWithFloatsOrNotAppl, notApplicPattern
):  # If you just print a normal list, the internal string-floats are printed with quotes ' '. To avoit it we use this.
...
def getMeanOfListExclNA(list1, notApplicPattern):
    ...
    return addedValuesForMeanCalc / float(
        numberOfApplicableEntries) if numberOfApplicableEntries > 0 else notApplicPattern
...
def getMeanPerColOf2dListExclNA(list2d, notApplicPattern):
    ...
    for col_i in xrange(len(list2d[0])):
        listColumn = [list2d[i][col_i] for i in xrange(len(list2d))
                      ]  # make a list of a column's elements.
        meanOfColumn = getMeanOfListExclNA(
            listColumn,
            notApplicPattern)  #will return NA-pattern if all elements are NA.
        listWithMeanPerColumn.append(meanOfColumn)
    return listWithMeanPerColumn


def calculateTheZeroIntensityOf3dImage(image3d):
    intensityZeroOfChannel = np.mean([image3d[0, 0, 0], image3d[
        -1, 0, 0], image3d[0, -1, 0], image3d[-1, -1, 0], image3d[
            0, 0, -1], image3d[-1, 0, -1], image3d[0, -1, -1], image3d[-1, -1,
                                                                       -1]])
...
def calculateDiceCoefficient(predictedBinaryLabels, groundTruthBinaryLabels):
    ...
    diceCoeff = (2.0 * numberOfTruePositives) / (
        np.sum(predictedBinaryLabels) +
        numberOfGtPositives) if numberOfGtPositives <> 0 else -1
    return diceCoeff
...
def get_random_subject_indices_to_load_on_GPU(
    ...
    if max_subjects_on_gpu_for_subepoch >= total_number_of_subjects:
        ...
        if get_max_subjects_for_gpu_even_if_total_less:  #This is if I want to have a certain amount on GPU, even if total subjects are less.
            ...
            if len(random_order_chosen_subjects) <> max_subjects_on_gpu_for_subepoch:
                if myLogger <> None:
                    myLogger.print3(
                        "ERROR: in get_random_subjects_indices_to_load_on_GPU(), something is wrong!")
                else:
...
def getSuggestedStdForSubsampledImage(subsampleFactor):
    ...
    suggestedStdsForSubsampledChannels = suggestedStdsForSubsampledChannels * (
        arraySubsampledFactor <> 1)
    return suggestedStdsForSubsampledChannels


    #This is the generic function.
def saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(
    ...
    fullFilenameToSaveWith = os.path.abspath(
        fullFilenameToSaveWith)  # Cleans the .././/...
    img_proxy_for_orig_image = nib.load(
        fullFilenameOfOriginalImageToCopyHeader)
    hdr_for_orig_image = img_proxy_for_orig_image.header

    affine_trans_to_ras = img_proxy_for_orig_image.affine

    newLabelImg = nib.Nifti1Image(
        labelImageCreatedByPredictions,
        affine_trans_to_ras)  #Nifti Constructor. data is the image itself, dimensions x,y,z,time. The second argument is the affine RAS transf.
    newLabelImg.set_data_dtype(npDtype)

    dimensionsOfTheGivenArrayImageToSave = len(
        labelImageCreatedByPredictions.shape)
    newZooms = list(hdr_for_orig_image.get_zooms(
    )[:dimensionsOfTheGivenArrayImageToSave])
    if len(newZooms) < dimensionsOfTheGivenArrayImageToSave:  #Eg if original image was 3D, but I need to save a multichannel image.
        newZooms = newZooms + [1.0] * (dimensionsOfTheGivenArrayImageToSave -
                                       len(newZooms))
...
def savePredictedImageToANewNiiWithHeaderFromOther(labelImageCreatedByPredictions,
    ...
    if myLogger <> None:
        myLogger.print3(
            "Saving the new label (segmentation) image for the subject #" +
            str(case_i))
    else:
        print("Saving the new label (segmentation) image for the subject #" +
              str(case_i))

    fullFilenameOfOriginalImageToCopyHeader = listOfFilepathsToEachChannelOfEachPatient[
        case_i][0]

    fullFilenameToSaveWith = "PLACEHOLDER"
    if listOfNamesToGiveToPredictions[case_i].endswith(".nii.gz"):
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[
            case_i][:-7] + suffixToAdd + ".nii.gz"
    elif listOfNamesToGiveToPredictions[case_i].endswith(".nii"):
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[
            case_i][:-4] + suffixToAdd + ".nii.gz"
    else:
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[
            case_i] + suffixToAdd + ".nii.gz"

    saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(
        labelImageCreatedByPredictions, fullFilenameToSaveWith,
        fullFilenameOfOriginalImageToCopyHeader, npDtype, myLogger)


def saveFmActivationImageToANewNiiWithHeaderFromOther(
        fmImageCreatedByVisualisation,
        listOfNamesToGiveToPredictions,
        listOfFilepathsToEachChannelOfEachPatient,
        image_i,
    ...
    saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(
        fmImageCreatedByVisualisation, fullFilenameToSaveWith,
        fullFilenameOfOriginalImageToCopyHeader, np.dtype(np.float32),
        myLogger)


def saveMultidimensionalImageWithAllVisualisedFmsToANewNiiWithHeaderFromOther(
        multidimImageWithAllVisualisedFms,
    ...
    stringToPrint = "Saving a multi-dimensional image, with all the FMs as a 4th dimension, for the subject #" + str(
        image_i)
    if myLogger <> None:
        myLogger.print3(stringToPrint)
    else:
    ...
    fullFilenameOfOriginalImageToCopyHeader = listOfFilepathsToEachChannelOfEachPatient[
        image_i][0]
    fullFilenameToSaveWith = "PLACEHOLDER"
    if listOfNamesToGiveToFmVisualisationsIfSaving[image_i].endswith(
            ".nii.gz"):
        fullFilenameToSaveWith = listOfNamesToGiveToFmVisualisationsIfSaving[
            image_i][:-7] + "_allFmsMultiDim.nii.gz"
    elif listOfNamesToGiveToFmVisualisationsIfSaving[image_i].endswith(".nii"):
        fullFilenameToSaveWith = listOfNamesToGiveToFmVisualisationsIfSaving[
            image_i][:-4] + "_allFmsMultiDim.nii.gz"
    else:
        fullFilenameToSaveWith = listOfNamesToGiveToFmVisualisationsIfSaving[
            image_i] + "_allFmsMultiDim.nii.gz"

    saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(
        multidimImageWithAllVisualisedFms, fullFilenameToSaveWith,
        fullFilenameOfOriginalImageToCopyHeader, np.dtype(np.float32),
        myLogger)
