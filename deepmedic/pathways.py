...
def cropRczOf5DimArrayToMatchOther(array5DimToCrop,
                                   dimensionsOf5DimArrayToMatchInRcz):
    # dimensionsOf5DimArrayToMatchInRcz : [ batch size, num of fms, r, c, z]
    output = array5DimToCrop[:, :, :dimensionsOf5DimArrayToMatchInRcz[
        2], :dimensionsOf5DimArrayToMatchInRcz[3], :
...
def upsampleRcz5DimArrayAndOptionalCrop(
        array5dimToUpsample,
        upsamplingFactor,
    ...
    if upsamplingScheme == "repeat":
        upsampledOutput = repeatRcz5DimArrayByFactor(array5dimToUpsample,
                                                     upsamplingFactor)
    else:
    ...
    if dimensionsOf5DimArrayToMatchInRcz <> None:
        ...
        output = cropRczOf5DimArrayToMatchOther(
            upsampledOutput, dimensionsOf5DimArrayToMatchInRcz)
    else:
...
def getMiddlePartOfFms(fms, listOfNumberOfCentralVoxelsToGetPerDimension):
    ...
    fmsShape = T.shape(
        fms)  #fms.shape works too, but this is clearer theano grammar.
    ...
    rIndexToStopGettingCentralVoxels = rIndexToStartGettingCentralVoxels + listOfNumberOfCentralVoxelsToGetPerDimension[
        0]  # Excluding
    cCentreOfPartIndex = (fmsShape[3] - 1) / 2
    cIndexToStartGettingCentralVoxels = cCentreOfPartIndex - (
        listOfNumberOfCentralVoxelsToGetPerDimension[1] - 1) / 2
    cIndexToStopGettingCentralVoxels = cIndexToStartGettingCentralVoxels + listOfNumberOfCentralVoxelsToGetPerDimension[
        1]  # Excluding

    if len(listOfNumberOfCentralVoxelsToGetPerDimension) == 2:  # the input FMs are of 2 dimensions (for future use)
        return fms[:, :, rIndexToStartGettingCentralVoxels:
                   rIndexToStopGettingCentralVoxels,
                   cIndexToStartGettingCentralVoxels:
                   cIndexToStopGettingCentralVoxels]
    elif len(
            listOfNumberOfCentralVoxelsToGetPerDimension) == 3:  # the input FMs are of 3 dimensions
        zCentreOfPartIndex = (fmsShape[4] - 1) / 2
        zIndexToStartGettingCentralVoxels = zCentreOfPartIndex - (
            listOfNumberOfCentralVoxelsToGetPerDimension[2] - 1) / 2
        zIndexToStopGettingCentralVoxels = zIndexToStartGettingCentralVoxels + listOfNumberOfCentralVoxelsToGetPerDimension[
            2]  # Excluding
        return fms[:, :, rIndexToStartGettingCentralVoxels:
                   rIndexToStopGettingCentralVoxels,
                   cIndexToStartGettingCentralVoxels:
                   cIndexToStopGettingCentralVoxels,
                   zIndexToStartGettingCentralVoxels:
                   zIndexToStopGettingCentralVoxels]
...
def makeResidualConnectionBetweenLayersAndReturnOutput(
    ...
    if np.any(np.asarray(deeperLayerOutputImageShapeTrain[2:]) > np.asarray(earlierLayerOutputImageShapeTrain[2:])) or \
        ...
        myLogger.print3("\t (train) Dimensions of Deeper Layer=" + str(
            deeperLayerOutputImageShapeTrain) +
                        ". Dimensions of Earlier Layer=" + str(
                            earlierLayerOutputImageShapeTrain))
        myLogger.print3("\t (val) Dimensions of Deeper Layer=" + str(
            deeperLayerOutputImageShapeVal) + ". Dimensions of Earlier Layer="
                        + str(earlierLayerOutputImageShapeVal))
        myLogger.print3("\t (test) Dimensions of Deeper Layer=" + str(
            deeperLayerOutputImageShapeTest) + ". Dimensions of Earlier Layer="
...
class Pathway(object):
    ...
    def makeLayersOfThisPathwayAndReturnDimensionsOfOutputFM(self,
        ...
        self._setInputAttributes(inputTrain, inputVal, inputTest,
                                 inputDimsTrain, inputDimsVal, inputDimsTest)
        ...
        for layer_i in xrange(0, numOfLayers):
            thisLayerFilterShape = [numKernsPerLayer[layer_i],
                                    inputToNextLayerShapeTrain[1]
                                    ] + kernelDimsPerLayer[layer_i]

            thisLayerUseBn = useBnPerLayer[layer_i]
            thisLayerActivFunc = activFuncPerLayer[
                layer_i]  # 0 relu, 1 prelu, -1 linear (no activ, for 1st layer over raw input).
            thisLayerDropoutRate = dropoutRatesPerLayer[
                layer_i] if dropoutRatesPerLayer else 0

            thisLayerPoolingParameters = poolingParamsStructureForThisPathwayType[
                layer_i]

            myLogger.print3("\t[Conv.Layer_" + str(layer_i) +
                            "], Filter Shape: " + str(thisLayerFilterShape))
            ...
            if layer_i not in indicesOfLayersToConnectResidualsInOutputForPathway:  #not a residual connecting here
                ...
                myLogger.print3("\t[Pathway_" + str(self.getStringType(
                )) + "]: making Residual Connection between output of [Layer_"
                                + str(
                                    layer_i) + "] to input of previous layer.")
                deeperLayerOutputImagesTrValTest = (
                    layer.outputTrain, layer.outputVal, layer.outputTest)
                deeperLayerOutputImageShapesTrValTest = (
                    layer.outputShapeTrain, layer.outputShapeVal,
                    layer.outputShapeTest)
                ...
                earlierLayerOutputImagesTrValTest = (earlierLayer.inputTrain,
                                                     earlierLayer.inputVal,
                                                     earlierLayer.inputTest)
                earlierLayerOutputImageShapesTrValTest = (
                    earlierLayer.inputShapeTrain, earlierLayer.inputShapeVal,
                    earlierLayer.inputShapeTest)

                (inputToNextLayerTrain, inputToNextLayerVal,
                 inputToNextLayerTest
                 ) = makeResidualConnectionBetweenLayersAndReturnOutput(
                     myLogger, deeperLayerOutputImagesTrValTest,
                     deeperLayerOutputImageShapesTrValTest,
                     earlierLayerOutputImagesTrValTest,
                     earlierLayerOutputImageShapesTrValTest)
        ...
        self._setOutputAttributes(
            inputToNextLayerTrain, inputToNextLayerVal, inputToNextLayerTest,
            inputToNextLayerShapeTrain, inputToNextLayerShapeVal,
    ...
    def makeMultiscaleConnectionsForLayerType(
            self, convLayersToConnectToFirstFcForMultiscaleFromThisLayerType):

        layersInThisPathway = self.getLayers()

        [outputOfPathwayTrain, outputOfPathwayVal,
         outputOfPathwayTest] = self.getOutput()
        [outputShapeTrain, outputShapeVal,
         outputShapeTest] = self.getShapeOfOutput()
        ...
        for convLayer_i in convLayersToConnectToFirstFcForMultiscaleFromThisLayerType:
            ...
            middlePartOfFmsTrain = getMiddlePartOfFms(
                thisLayer.outputTrain, numOfCentralVoxelsToGetTrain)
            middlePartOfFmsVal = getMiddlePartOfFms(thisLayer.outputVal,
                                                    numOfCentralVoxelsToGetVal)
            middlePartOfFmsTest = getMiddlePartOfFms(
                thisLayer.outputTest, numOfCentralVoxelsToGetTest)

            outputOfPathwayTrain = T.concatenate(
                [outputOfPathwayTrain, middlePartOfFmsTrain],
                axis=1)
            outputOfPathwayVal = T.concatenate(
                [outputOfPathwayVal, middlePartOfFmsVal],
                axis=1)
            outputOfPathwayTest = T.concatenate(
                [outputOfPathwayTest, middlePartOfFmsTest],
                axis=1)
        ...
        self._setOutputAttributes(outputOfPathwayTrain, outputOfPathwayVal,
                                  outputOfPathwayTest, outputShapeTrain,
    ...
    def calcRecFieldOfPathway(self, kernelDimsPerLayer):
        return calculateReceptiveFieldDimensionsFromKernelsDimListPerLayerForFullyConvCnnWithStrides1(
            kernelDimsPerLayer)

    def calcInputRczDimsToProduceOutputFmsOfCompatibleDims(
            self, thisPathWayKernelDims, dimsOfOutputFromPrimaryPathway):
        recFieldAtEndOfPathway = self.calcRecFieldOfPathway(
            thisPathWayKernelDims)
        ...
        for rcz_i in xrange(3):
            rczDimsOfOutputOfPathwayShouldBe[rcz_i] = int(ceil(
                rczDimsOfOutputFromPrimaryPathway[rcz_i] / (
                    1.0 * self.subsFactor()[rcz_i])))
            rczDimsOfInputToPathwayShouldBe[rcz_i] = recFieldAtEndOfPathway[
                rcz_i] + rczDimsOfOutputOfPathwayShouldBe[rcz_i] - 1
        return rczDimsOfInputToPathwayShouldBe

        # Setters
    def _setInputAttributes(self, inputToLayerTrain, inputToLayerVal,
    ...
    def getShapeOfOutput(self):
        return [self._outputShapeTrain, self._outputShapeVal,
                self._outputShapeTest]

    def getShapeOfInput(self):
        return [self._inputShapeTrain, self._inputShapeVal,
                self._inputShapeTest]
...
class SubsampledPathway(Pathway):
    ...
    def upsampleOutputToNormalRes(self,
                                  upsamplingScheme="repeat",
                                  shapeToMatchInRczTrain=None,
                                  shapeToMatchInRczVal=None,
                                  shapeToMatchInRczTest=None):
        ...
        outputNormResTrain = upsampleRcz5DimArrayAndOptionalCrop(
            outputTrain, self.subsFactor(), upsamplingScheme,
            shapeToMatchInRczTrain)
        outputNormResVal = upsampleRcz5DimArrayAndOptionalCrop(
            outputVal, self.subsFactor(), upsamplingScheme,
            shapeToMatchInRczVal)
        outputNormResTest = upsampleRcz5DimArrayAndOptionalCrop(
            outputTest, self.subsFactor(), upsamplingScheme,
            shapeToMatchInRczTest)

        outputNormResShapeTrain = outputShapeTrain[:
                                                   2] + shapeToMatchInRczTrain[
                                                       2:]
        outputNormResShapeVal = outputShapeVal[:2] + shapeToMatchInRczVal[2:]
        outputNormResShapeTest = outputShapeTest[:2] + shapeToMatchInRczTest[
            2:]

        self._setOutputAttributesNormRes(
            outputNormResTrain, outputNormResVal, outputNormResTest,
            outputNormResShapeTrain, outputNormResShapeVal,
            outputNormResShapeTest)

    def _setOutputAttributesNormRes(self, outputNormResTrain, outputNormResVal,
                                    outputNormResTest, outputNormResShapeTrain,
                                    outputNormResShapeVal,
                                    outputNormResShapeTest):
    ...
    def getOutputAtNormalRes(self):
        ...
        return [self._outputNormResTrain, self._outputNormResVal,
                self._outputNormResTest]

    def getShapeOfOutputAtNormalRes(self):
        # upsampleOutputToNormalRes() must be called first once.
        return [self._outputNormResShapeTrain, self._outputNormResShapeVal,
                self._outputNormResShapeTest]
