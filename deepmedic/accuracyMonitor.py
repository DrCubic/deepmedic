import numpy as np
from deepmedic.genericHelpers import (getMeanOfListExclNA, strFl4fNA,
                                      strFl5fNA, strListFl4fNA, strListFl5fNA)
...
class AccuracyOfEpochMonitorSegmentation(object):
    ...
    def __init__(self,
        ...
        if self.training0orValidation1 == 0:
            ...
            ]  # mean value of the cost function (training only)
        self.correctlyPredVoxelsInEachSubep = []
        self.numberOfAllSamplesOfEachSubep = []
        self.meanEmpiricalAccuracyOfEachSubep = [
        ]  # mean (multiclass) accuracy. correctlyPredVoxelsInEachSubep / numberOfAllSamplesOfEachSubep # NOTE: Cannot contain N0T-APPLICABLE
        ...
        ]  # subepochs X Classes X 4. 4 = RP, RN, TP, TN
        ...
        # ==== API ====
    def getMeanEmpiricalAccuracyOfEpoch(
            self
    ...
    def updateMonitorAccuraciesWithNewSubepochEntries(
            self,
            meanCostOfSubepoch,
        ...
        # Predicted with the correct class.
        for class_i in xrange(self.numberOfClasses):
            correctlyPredVoxelsInSubep += perClassRpRnTpTnInSubep[class_i][
                2]  # Add up the true positives per class (which are one vs all).
        self.correctlyPredVoxelsInEachSubep.append(correctlyPredVoxelsInSubep)
        numberOfAllSamples = perClassRpRnTpTnInSubep[
            0, 0] + perClassRpRnTpTnInSubep[
                0, 1]  #RealPos + RealNeg wrt any class (eg backgr)
        ...
        ]  # Classes x 4 . 4=Mean Acc, sens, spec, dsc.
        for class_i in xrange(self.numberOfClasses):
            ...
            meanAccuracyClassVsAllOfSubep = (
                numOfTruePosInSubep + numOfTrueNegInSubep) / float(
                    numOfRealPosInSubep + numOfRealNegInSubep)
            ...
            meanDiceOfSubep = self.NA_PATTERN if numOfRealPosInSubep == 0 else (
                2.0 * numOfTruePosInSubep) / (
                    numOfPredPosInSubep + numOfRealPosInSubep)

            listWithPerClassMeanAccSensSpecDscInSubep.append(
                [meanAccuracyClassVsAllOfSubep, meanAccuracyOnPosOfSubep,
                 meanAccuracyOnNegOfSubep, meanDiceOfSubep])
        self.listPerSubepPerClassMeanAccSensSpecDsc.append(
            listWithPerClassMeanAccSensSpecDscInSubep)
        ...
        self.listPerSubepForegrRpRnTpTn.append([foregrRp, foregrRn, foregrTp,
                                                foregrTn])

        foregrMeanAccOfSubep = (
        ...
        foregrMeanDiceOfSubep = self.NA_PATTERN if foregrRp == 0 else (
            2.0 * foregrTp) / (foregrPredPosInSubep + foregrRp)
        self.listPerSubepForegrMeanAccSensSpecDsc.append(
            [foregrMeanAccOfSubep, foregrMeanAccOnPosOfSubep,
    ...
    def reportAccuracyForLastSubepoch(self):
        ...
        logStr = trainOrValString + ": Epoch #" + str(
            self.epoch) + ", Subepoch #" + str(currSubep)
        ...
        if self.training0orValidation1 == 0:  # During training, also report the mean value of the Cost Function:
            self.logger.print3(logStr + ", Overall:\t mean cost:      \t" +
                               strFl5fNA(self.meanCostOfEachSubep[currSubep],
                                         self.NA_PATTERN))
        ...
        for class_i in xrange(self.numberOfClasses):
            ...
            self.logger.print3(
                "+++++++++++++++ Reporting Accuracy over whole subepoch for " +
            ...
            self.logger.print3(
                logStrClass + "\t mean accuracy:   \t" + strFl4fNA(
                    meanAccClassOfSubep, self.NA_PATTERN) +
                "\t=> (TruePos+TrueNeg)/All-Predicted-Voxels = " + str(
                    numOfTpInSubep + numOfTnInSubep) + "/" + str(
                        numOfRpInSubep + numOfRnInSubep))
            self.logger.print3(logStrClass + "\t mean sensitivity:\t" +
                               strFl4fNA(meanAccOnPosOfSubep, self.NA_PATTERN)
                               + "\t=> TruePos/RealPos = " + str(
                                   numOfTpInSubep) + "/" + str(numOfRpInSubep))
            self.logger.print3(logStrClass + "\t mean specificity:\t" +
                               strFl4fNA(meanAccOnNegOfSubep, self.NA_PATTERN)
                               + "\t=> TrueNeg/RealNeg = " + str(
                                   numOfTnInSubep) + "/" + str(numOfRnInSubep))
            self.logger.print3(logStrClass + "\t mean Dice:       \t" +
                               strFl4fNA(meanDiceOfSubep, self.NA_PATTERN))
    ...
    def reportMeanAccyracyOfEpoch(self):
        ...
        self.logger.print3(
            logStr + ", Overall:\t mean accuracy of epoch:\t" + strFl4fNA(
                meanEmpiricalAccOfEp, self.NA_PATTERN) +
            "\t=> Correctly-Classified-Voxels/All-Predicted-Voxels")
        if self.training0orValidation1 == 0:  # During training, also report the mean value of the Cost Function:
            meanCostOfEp = getMeanOfListExclNA(self.meanCostOfEachSubep,
                                               self.NA_PATTERN)
            self.logger.print3(
                logStr + ", Overall:\t mean cost of epoch:    \t" + strFl5fNA(
                    meanCostOfEp, self.NA_PATTERN))

        self.logger.print3(logStr +
        ...
        for class_i in xrange(self.numberOfClasses):
            ...
            logStrClass = logStr + ", " + classString + ":"
            self.logger.print3(logStrClass + "\t mean accuracy of epoch:\t" +
                               strFl4fNA(meanAccOfEp, self.NA_PATTERN) +
                               "\t=> (TruePos+TrueNeg)/All-Predicted-Voxels")
            self.logger.print3(
                logStrClass + "\t mean sensitivity of epoch:\t" + strFl4fNA(
                    meanSensOfEp, self.NA_PATTERN) + "\t=> TruePos/RealPos")
            self.logger.print3(
                logStrClass + "\t mean specificity of epoch:\t" + strFl4fNA(
                    meanSpecOfEp, self.NA_PATTERN) + "\t=> TrueNeg/RealNeg")
            self.logger.print3(logStrClass + "\t mean Dice of epoch:    \t" +
                               strFl4fNA(meanDscOfEp, self.NA_PATTERN))

            #Visualised in my scripts:
            self.logger.print3(logStrClass +
                               "\t mean accuracy of each subepoch:\t" +
                               strListFl4fNA(meanAccPerSubep, self.NA_PATTERN))
            self.logger.print3(
                logStrClass + "\t mean sensitivity of each subepoch:\t" +
                strListFl4fNA(meanSensPerSubep, self.NA_PATTERN))
            self.logger.print3(
                logStrClass + "\t mean specificity of each subepoch:\t" +
