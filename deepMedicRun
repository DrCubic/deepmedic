...
def setupArgParser():
    ...
    parser.add_argument(
        ...
        dest='model_conf',
        type=str,
        help="Create a new CNN model with model parameters at given config file [MODEL_CONF].")
...
def checkDevicePassedCorrectly(devArg):
    if devArg <> "cpu" and devArg <> "gpu" and not (devArg.startswith("gpu")
                                                    and strIsInt(devArg[3:])):
        print(
            "ERROR: Value for the [-dev] option was not specified correctly. Specify the device to run the process on. Values: [cpu] or [gpu] (Default = cpu).\nIn the case of multiple GPUs, specify a particular GPU device with a number, in the format: gpu2. Exiting.")
        exit(1)
...
def setEnvironment(deviceString):
    ...
    os.environ[
        'THEANO_FLAGS'] = 'on_unused_input=warn, mode=FAST_RUN,device=' + deviceString + ',floatX=float32,force_device=True'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        print(
            "For help on the usage of this program, please use the option -h.")
        exit(1)
    ...
    if args.model_conf and args.saved_model:
        print(
            "ERROR:\t[-newModel] and [-model] options cannot be used together.\n\tUse [-newModel] to create a new cnn model.\n\t[-model] can follow a [-train] or [-test] option, to specify which saved model to use.\n\tPlease try -h for more information. Exiting.")
        exit(1)
    if args.testing_conf and (args.model_conf or args.training_conf):
        print(
            "ERROR: [-test] cannot be used in conjuction with [-newModel] or [-train]. To test with an existing network, please just specify a configuration file for the testing process, which will include a path to a trained model to use for the inference. Exiting.")
        exit(1)
    if args.pretrained_model and not args.model_conf:
        print(
            "ERROR: The option [-pretrained] can only be used together with the [-newModel] option.\n\tPlease try -h for more information. Exiting.")
        exit(1)
    if args.layers_to_transfer and not args.pretrained_model:
        print(
            "ERROR: The option [-layers] can only be used together with the [-pretrained] option.\n\tPlease try -h for more information. Exiting.")
        exit(1)
    if args.reset_optimizer and not args.training_conf:
        print(
            "ERROR: The option [-resetOptimizer] can only be used together with the [-train] option.\n\tPlease try -h for more information. Exiting.")
        exit(1)
    ...
    if args.model_conf:  #and not args.training_conf and not args.testing_conf:
        ...
        absPathToModelConf = getAbsPathEvenIfRelativeIsGiven(args.model_conf,
                                                             cwd)
        absPathToPreTrainedModelGivenInCmdLine = getAbsPathEvenIfRelativeIsGiven(
            args.pretrained_model,
            cwd) if args.pretrained_model else None  # pretraining.
        ...
        (cnnInstanceLoaded, filenameAndPathWhereNewlyCreatedModelWasSaved
         ) = deepMedicNewModel.deepMedicNewModelMain(
             absPathToModelConf, absPathToPreTrainedModelGivenInCmdLine,
    ...
    if args.training_conf:  #and not args.model_conf and not args.testing_conf: #Second part was checked already.
        ...
        absPathToTrainConf = getAbsPathEvenIfRelativeIsGiven(
            args.training_conf, cwd)
        absPathToCnnModelGivenInCmdLine = getAbsPathEvenIfRelativeIsGiven(
            args.saved_model, cwd) if args.saved_model else None
        deepMedicTrain.deepMedicTrainMain(
            absPathToTrainConf, absPathToCnnModelGivenInCmdLine,
            cnnInstanceLoaded, filenameAndPathWhereNewlyCreatedModelWasSaved,
    ...
    if args.testing_conf:  #and not args.model_conf and not args.training_conf : #Second part was checked already.
        ...
        absPathToTestConf = getAbsPathEvenIfRelativeIsGiven(args.testing_conf,
                                                            cwd)
        absPathToCnnModelGivenInCmdLine = getAbsPathEvenIfRelativeIsGiven(
            args.saved_model, cwd) if args.saved_model else None
        deepMedicTest.deepMedicTestMain(absPathToTestConf,
                                        absPathToCnnModelGivenInCmdLine)
